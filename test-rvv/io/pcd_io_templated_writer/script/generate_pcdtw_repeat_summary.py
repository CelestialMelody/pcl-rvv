#!/usr/bin/env python3
"""Summarize repeated pcd_io_templated_writer board bench logs.

本脚本读取 run*/run_bench_std.log 和 run*/run_bench_rvv.log，生成只含
summary（摘要）的 Markdown。raw log 默认留在 topic-local log/board 下，
不进入提交。
"""
...
def parse_log(path: Path) -> dict:
    ...
    while index < len(lines):
        ...
        if warmup is None:
            match = WARMUP_RE.search(lines[index])
            if match:
...
def main() -> int:
    ...
    parser.add_argument("--title", default="pcd_io_templated_writer repeated board summary")
