...
在本机收集交叉编译测试所需的 `.so`，`strip` 后通过 `rsync` 同步到远端板卡，便于在硬件上直接运行已部署的可执行文件（配合各子目录的 `Makefile.board`）：
...
### 与子目录 `Makefile.board` 的配合

板卡上运行测试时，`LD_LIBRARY_PATH` 必须包含上述同步目录。请把各子目录 **`Makefile.board`** 里的 **`REMOTE_LIBS_DIR`**（或等价变量）设为与 **`REMOTE_LIB_DIR`** **一致**（注意默认可能是 `lib` 与 `libs` 等路径名差异，需自行统一）。
