# SampleConsensusModel (countWithinDistance)测试文档
...
两个测试用例（`SampleConsensusModelNormalPlane`和`SampleConsensusModelPlane`）均用于验证`countWithinDistance`方法的SIMD（单指令多数据）实现与标准实现的一致性，并测试不同指令集（SSE、AVX、RVV）的性能表现。测试通过多次迭代生成随机点云数据，分别使用不同指令集计算符合距离阈值的点数量，并对比结果与性能。
...
| 特性         | `SampleConsensusModelNormalPlane`（几何+角度距离）           | `SampleConsensusModelPlane`（纯几何距离） |
| ------------ | ------------------------------------------------------------ | ----------------------------------------- |
| 距离计算方式 | 综合考虑**几何距离**（点到平面的空间距离）和**角度距离**（法向量夹角），通过`normal_distance_weight`控制权重 | 仅计算**几何距离**（点到平面的垂直距离）  |
| 输入数据     | 需要点云数据（`PointXYZ`）和法向量数据（`Normal`）           | 仅需要点云数据（`PointXYZ`）              |
| 模型参数     | 除平面方程系数外，还需设置`normal_distance_weight`参数       | 仅需平面方程系数                          |
| 应用场景     | 适用于对法向量方向敏感的平面检测任务，能更精确地筛选符合条件的点 | 适用于仅关注空间位置的平面检测任务        |
