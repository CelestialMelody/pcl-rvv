...
/*
 * 本文件做什么：
 * 这里保存 CPPF `alpha_m` 的 closed-form audit（闭式公式审计）和 test-only
 * RVV candidate（测试专用 RVV 候选）。它把 production 当前每个 pair 都构造
 * Eigen `AngleAxisf` / `Affine3f` 的标量链路，改写成只计算旋转后 y/z 分量的
 * Rodrigues rotation formula（罗德里格旋转公式），再批量计算 `atan2`。
 *
 * 证据边界：
 * 这是 component ablation（组件消融）候选，不是 production dispatch（生产分流）。
 * 它只能说明 `alpha_m` 后段是否值得继续做有界生产探针，不能替代真实 public entry
 * （公开入口）证据。
 */
...
inline float
computeAlphaMClosedForm(const PointT& reference_point,
...
computeAlphaMClosedFormRVV(const AlphaMStaging& staging, std::vector<float>& alpha_m)
{
  ...
  for (std::size_t offset = 0; offset < pair_count;)
  {
    ...
    const vfloat32m2_t yz_norm2 =
        __riscv_vfmacc_vv_f32m2(__riscv_vfmul_vv_f32m2(ny, ny, vl), nz, nz, vl);
    ...
    const vfloat32m2_t scale =
        __riscv_vfdiv_vv_f32m2(__riscv_vfsub_vv_f32m2(one, nx, vl), yz_norm2_safe, vl);
    ...
    const vfloat32m2_t sin_angle =
        __riscv_vfsqrt_v_f32m2(__riscv_vfmax_vf_f32m2(
            __riscv_vfsub_vv_f32m2(one, __riscv_vfmul_vv_f32m2(nx, nx, vl), vl),
            0.0f,
            vl),
            vl);
...
inline void
computeCPPFAlphaMBatchRVV(const CloudT& cloud,
