...
- title：RangeImageBorderExtractor RangeImage score-generation-plus-update production-shaped diagnostic repeated board summary
- evidence_role：production-shaped diagnostic
- summary_path：log/board/repeated_phase020_range_image_generation_score_update/summary.md
- comparisons：2
- result：Errors=0，Warnings=2，Suggestions=4
...
### 1. ba_degradation_frequency — range_image_generation_plus_update_160x120

- observed_pattern：B/A values=1.01x, 0.99x, 1x, 1x, 1x；1/5 低于 1。
...
### 2. ba_degradation_frequency — range_image_score_generation_160x120

- observed_pattern：B/A values=1x, 0.99x, 1x, 1x, 1x；1/5 低于 1。
- why_suspicious：平均或 median 正向不能掩盖退化频率；高频退化说明候选不稳定或 case 分布被少数值主导。
- possible_non_bug_explanations：可能是测量噪声、温度 / 频率波动、输入局部性变化或 run 数不足。
- possible_bug_or_evidence_issues：也可能是候选在某些点型、规模、row source、mask 或 reduction 路径上真实退化。
- recommended_checks：报告退化频率；必要时扩大 runs，按 point type / size / row source 分组，检查 trace 和 asm attribution。
...
### 1. environment_metadata_missing — range_image_generation_plus_update_160x120
...
### 2. near_threshold_ba — range_image_generation_plus_update_160x120
...
### 3. environment_metadata_missing — range_image_score_generation_160x120
...
### 4. near_threshold_ba — range_image_score_generation_160x120
