...
- title：sac_model_circle3d Phase 010 selectWithinDistance production repeated board summary
- evidence_role：production-public
- summary_path：test-rvv/sample_consensus/sac_model_circle3d/doc/phases/010-selectwithin-production-probe/select-production-repeated-evidence-manifest.json
- comparisons：1
...
### 1. ba_degradation_frequency — sac_model_circle3d selectWithinDistance production public Std/RVV PointXYZ 65536

- observed_pattern：B/A values=1.06x, 0.907x, 1.04x, 0.992x, 1.28x, 1.07x, 1.01x, 0.966x, 0.843x, 0.587x；5/10 低于 1。
...
### 1. long_tail_or_variance — sac_model_circle3d selectWithinDistance production public Std/RVV PointXYZ 65536

- observed_pattern：min=0.587x, median=0.999x, max=1.28x，max/min=2.19。
