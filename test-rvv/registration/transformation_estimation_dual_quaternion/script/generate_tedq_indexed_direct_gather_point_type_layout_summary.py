#!/usr/bin/env python3
"""生成 indexed direct gather 点型/layout 扩展的板卡摘要。

该脚本比较 Std/RVV 两个 bench binary 中的同名
`indexed-direct-gather-point-type-layout` case。它只覆盖 test-rvv diagnostic
边界：row_source_policy 为 source-indexed-cloud-pair 或 dual-indexed-cloud-pair，
point type 为 PointXYZI 或 PointXYZRGB。summary 和 manifest 不能替代 public-entry
direct evidence。
"""
...
POINT_TYPE_CASE_RE = re.compile(
    r"^direct gather candidate "
    r"(?P<row_source>source-indexed-cloud-pair|dual-indexed-cloud-pair) "
    r"(?P<point_type>PointXYZI|PointXYZRGB) (?P<size>\d+K)$"
)
...
def collect_labels(runs: list[dict[str, Any]]) -> list[tuple[str, str, str, str]]:
  labels: list[tuple[str, str, str, str]] = []
  for name in sorted(runs[0]["std"]["cases"]):
    match = POINT_TYPE_CASE_RE.match(name)
    if match:
      labels.append(
          (name, match.group("row_source"), match.group("point_type"), match.group("size"))
      )
  if not labels:
    raise SystemExit("no indexed direct gather point-type layout cases found")
  return labels


def build_metrics(runs: list[dict[str, Any]]) -> list[dict[str, Any]]:
  result: list[dict[str, Any]] = []
  for label, row_source, point_type, size in collect_labels(runs):
    values: list[float] = []
  ...
  return sorted(result, key=lambda item: (item["row_source"], item["point_type"], item["size"]))


def side(row_source: str, point_type: str, build: str, ms_key: str, ms: float, checksums: list[str]) -> dict[str, Any]:
  return {
      "label": build,
      "boundary": "test_support_indexed_direct_gather_point_type_layout",
      "wrapper": "bench_tedq",
      "row_source": row_source,
      "point_type": point_type,
      "solve": True,
      ...
      "timer_boundary": "indexed_gather_plus_c1_c2_accumulation_plus_eigen_4x4_solve",
      "gate": f"dense_{row_source}_{point_type}_float_xyz_aos",
      "mask": "not_applicable_dense_only",
      "reduction": "scalar_same_chain" if build == "Std build" else "rvv_f64_tree",
      "asm_boundary": "not_applicable_std_build" if build == "Std build" else "bench_binary_indexed_direct_gather",
      ms_key: ms,
  }
...
def build_manifest(runs: list[dict[str, Any]], metrics: list[dict[str, Any]], args: argparse.Namespace, summary_path: Path) -> dict[str, Any]:
  ...
  for item in metrics:
    ...
    comparisons.append(
        {
            "name": f"indexed direct gather point-type layout {item['row_source']} {item['point_type']} {item['size_label']}",
            "group": f"tedq_indexed_direct_gather_point_type_layout_{item['row_source']}",
            "case_kind": "point_type_layout_diagnostic",
            "evidence_role": "diagnostic",
            "row_source": item["row_source"],
            ...
            "notes": "B/A = Std direct gather ms / RVV direct gather ms for the same row source, point type and index corpus. This is diagnostic evidence, not public-entry evidence.",
            "baseline": side(item["row_source"], item["point_type"], "Std build", "std_ms", std_med, item["checksums"]["std"]),
            "candidate": side(item["row_source"], item["point_type"], "RVV build", "rvv_ms", rvv_med, item["checksums"]["rvv"]),
        }
    )
  return {
      "schema_version": 1,
      "summary": {
          ...
          "label": "source/dual indexed direct gather PointXYZI/PointXYZRGB layout diagnostic; test-rvv only",
          "analysis_script": "test-rvv/registration/transformation_estimation_dual_quaternion/script/generate_tedq_indexed_direct_gather_point_type_layout_summary.py",
          "metadata": {
...
def write_summary(path: Path, metrics: list[dict[str, Any]], args: argparse.Namespace, manifest_path: Path, doctor_path: Path) -> None:
  ...
  for item in metrics:
    ...
    lines.append(
        f"| {item['row_source']} | {item['point_type']} | {item['size_label']} | `{fmt(m['values'])}` | {m['median']:.3f} | {m['min']:.3f} | {m['max']:.3f} | `{m['bucket']}` | {checksum_note(item)} |"
    )
  lines.extend(
      [
          "",
...
def main() -> int:
  ...
  parser.add_argument("--input-dir", default="log/board/indexed_direct_gather_point_type_layout_repeated")
  parser.add_argument("--output-dir", default="log/board/indexed_direct_gather_point_type_layout_repeated")
  parser.add_argument("--run-label", default="indexed_direct_gather_point_type_layout_repeated")
  parser.add_argument("--case-filter", default="indexed-direct-gather-point-type-layout")
