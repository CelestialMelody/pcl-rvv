...
- summary_path：log/board/correspondence_point_type_layout_repeated/summary.md
- comparisons：6
- result：Errors=0，Warnings=5，Suggestions=0
...
### 1. long_tail_or_variance — correspondence point-type layout PointXYZI 64K

- observed_pattern：min=5.88x, median=6.69x, max=6.94x，max/min=1.18。
...
### 2. long_tail_or_variance — correspondence point-type layout PointXYZI 256K

- observed_pattern：min=4.04x, median=5.76x, max=6.68x，max/min=1.65。
- why_suspicious：长尾说明单一均值不足以描述稳定性，异常值可能改变接入判断。
- possible_non_bug_explanations：可能是板卡温度、频率、调度、缓存状态或 run 顺序导致。
- possible_bug_or_evidence_issues：也可能是候选存在数据相关分支、fallback、spill/reload 或内存带宽拐点。
- recommended_checks：查看 per-iteration trace、温度 / governor / freq / taskset；必要时扩大到 20-run 或 50-run。
...
### 3. long_tail_or_variance — correspondence point-type layout PointXYZRGB 64K

- observed_pattern：min=5.28x, median=6.14x, max=6.93x，max/min=1.31。
- why_suspicious：长尾说明单一均值不足以描述稳定性，异常值可能改变接入判断。
- possible_non_bug_explanations：可能是板卡温度、频率、调度、缓存状态或 run 顺序导致。
- possible_bug_or_evidence_issues：也可能是候选存在数据相关分支、fallback、spill/reload 或内存带宽拐点。
- recommended_checks：查看 per-iteration trace、温度 / governor / freq / taskset；必要时扩大到 20-run 或 50-run。
...
### 4. long_tail_or_variance — correspondence point-type layout PointXYZRGB 256K

- observed_pattern：min=2.96x, median=4.28x, max=5.74x，max/min=1.94。
- why_suspicious：长尾说明单一均值不足以描述稳定性，异常值可能改变接入判断。
- possible_non_bug_explanations：可能是板卡温度、频率、调度、缓存状态或 run 顺序导致。
- possible_bug_or_evidence_issues：也可能是候选存在数据相关分支、fallback、spill/reload 或内存带宽拐点。
- recommended_checks：查看 per-iteration trace、温度 / governor / freq / taskset；必要时扩大到 20-run 或 50-run。
...
### 5. group_outlier — correspondence point-type layout PointXYZRGB 256K

- observed_pattern：group=tedq_correspondence_point_type_layout 的组内 median=5.74x；当前 case median=4.28x，偏离 25.4%。
