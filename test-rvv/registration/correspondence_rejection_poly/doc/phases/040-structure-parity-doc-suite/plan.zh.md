...
| `benchmark-and-evidence` | 已列 case 和 evidence，但 registry / artifact tracking / production-direct 开关说明可更清晰 | 需要 CLI、case-filter、计时边界、checksum、QEMU / board、doctor、asm、复现命令和提交边界 | adopted by this phase | 无 blocker | 补 target 字典、提交白名单和 doctor 处理 |
...
| long-term `doc-rvv` | absent | 只有 adopted production behavior、production patch 或 PI5 生产证据闭环通过后适用 | not_applicable with evidence | EvidenceDecision 是 `rollback/no-production`，production diff clean | README / evaluation / Handoff 明确不创建 |
...
| 040-C 扩展 bench / evidence 文档 | `doc/benchmark-and-evidence.zh.md` | `CRPOLY_ENABLE_PRODUCTION_DIRECT_PROBE` 边界、Evidence Doctor、registry 和提交白名单清晰 |
