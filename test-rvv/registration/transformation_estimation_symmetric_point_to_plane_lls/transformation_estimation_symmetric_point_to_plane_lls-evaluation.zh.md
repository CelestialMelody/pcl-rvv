...
当前结论是 `production-ready/generic-normal-full-cloud`（泛型 normal 全云入口可生产接入）。第二轮 production integration loop（生产接入闭环）已把 production（生产源码）full-cloud（全云顺序扫描）公开入口从 exact `PointNormal` 扩展到 traits-gated generic normal point type（用 PCL 点类型字段特征约束的泛型 normal 点类型）：`PointSource` 和 `PointTarget` 必须分别满足 `x/y/z/normal_x/normal_y/normal_z` 都是单个 `float` 字段、POD（普通数据布局）/ standard-layout（标准布局）和 offset alignment（字段偏移对齐）条件，且 `Scalar=float`。indices、source+target indices、correspondences 和 `Scalar=double` 全部保持标量 fallback（回退路径）。

2026-07-22 小型公共化重构后，上述点类型 gate 改由 `pcl::rvv::RVVXYZNormalFloatLayout<PointT>` 提供，本地 `SymmetricXYZNormalFloatLayout` 已删除；32-bit byte offset 边界改用 `pcl::rvv::rvvMaxU32ByteOffsetElements<PointT>()`。本轮只移动编译期 traits/layout 判断，不改变 full-cloud dispatch（分流）范围，不把 indices、correspondences 或 `Scalar=double` 纳入 RVV production。

QEMU correctness（QEMU 正确性验证）通过，bench（性能测试）输出合同可解析，反汇编证明真实 public estimator（公开估计器）调用 production helper 模板 `estimateSymmetricPointNormalFullCloudRVV<PointSource, PointTarget>`。Milkv-Jupiter 板卡 `board_smoke` 18 个专项测试通过；production-direct full-cloud `PointNormal` 64K / 256K 为 `2.62x` / `2.47x`，`PointXYZINormal` 64K / 256K 为 `2.64x` / `1.89x`。correspondences diagnostic 仍退化为 `0.64x` / `0.41x`，不接 production，退化主因保持为待消融假设。
...
源码层面的数据流需要和诊断实现区分开看。production helper 只看到两个同步前进的 iterator；RVV 诊断为了暴露取数成本，显式拆成全云顺序扫描和对应关系索引扫描。前者使用 stride load（跨步加载）读取 AoS（结构数组）字段；后者先标量展开 source/target index，再用 gather 读取不连续点字段。
...
| 路径 | 当前处理 | 证据边界 |
| --- | --- | --- |
| 全云顺序扫描 | `n >= 64`、`vlmax_e32m2 <= 64` 且 32-bit byte offset 可表达时走 RVV。production 使用 traits offset 分别读取 source/target 坐标和 normal。 | production direct 已接入 generic normal / `float` full-cloud，板卡 `PointNormal` 为 `2.62x` / `2.47x`，`PointXYZINormal` 为 `2.64x` / `1.89x`。 |
| 对应关系索引 | 先展开 `src_indices/tgt_indices`，再用 `vluxei32.v` gather 点字段。 | QEMU correctness 通过，板卡退化，不接 production。 |
...
| 全云 production direct | 已接入 generic normal production | 板卡 `PointNormal` production-direct `2.62x` / `2.47x`，`PointXYZINormal` production-direct `2.64x` / `1.89x`，真实公开入口证据闭合。 | 仅覆盖 `Scalar=float` / full-cloud，且点类型必须满足 normal traits gate。 |
| correspondences production | 不接入 | 板卡 `0.64x` / `0.41x` 明确负向。 | 除非新增消融证明可避开 index 展开/gather 退化，否则保持标量。 |
...
| `SourceIndicesOverloadRemainsScalarFallback` | production fallback | source indices overload 保持标量。 |
...
QEMU `run_test_compare` 结果：std 构建 18 个测试通过，RVV 构建 18 个测试通过。板卡 `board_smoke` 中 RVV test 18 个测试通过。
...
- 输入 cloud、target 和 correspondences 在计时前构造完成。
...
| diagnostic full-cloud 64K | 7.8638 | 6.3597 | 1.24x | 诊断链路仍有收益。 |
| diagnostic correspondences 64K | 6.4343 | 10.0542 | 0.64x | indexed row 退化，不接 production。 |
...
| diagnostic correspondences 256K | 25.8557 | 62.3956 | 0.41x | 大规模 indexed row 仍退化。 |
...
- production helper `estimateSymmetricPointNormalFullCloudRVV<PointSource, PointTarget>` 模板符号存在，public full-cloud estimator 对 `PointNormal -> PointNormal` 和 `PointXYZINormal -> PointXYZINormal` 的调用点可见。
...
- 全二进制摘录可见 `vluxei32.v` gather，需 reviewer 用符号范围进一步复核是否全部来自 correspondences helper。
...
- 专项 diagnostic 已建，production 已接入 traits-gated generic normal full-cloud 分流。
- QEMU std/RVV 专项测试各 18 个通过，板卡 RVV test 18 个通过。
- QEMU bench 输出合同可解析，checksum 基本对齐。
- 反汇编证明真实 public full-cloud estimator 调用 `PointNormal` 和 `PointXYZINormal` production helper 模板实例，并揭示自动 partial vector accumulation。
- 板卡 production-direct `PointNormal` full-cloud 64K / 256K 为 `2.62x` / `2.47x`；`PointXYZINormal` full-cloud 64K / 256K 为 `2.64x` / `1.89x`。

证据不支持：

- correspondences 路径板卡 `0.64x` / `0.41x`，不能接入生产。
- 不支持扩大到 `Scalar=double`、indices 或 correspondences。
- 自动 `vfredosum.vs` 规约改变累加树；当前由 production direct 测试和板卡 bench 覆盖，但仍需作为数值边界记录。

第二轮按用户要求完成泛型 normal 扩展的 PI1-PI5。最终接入范围限定为全云连续 generic normal / `float` 入口；correspondences 和 indexed 入口保持标量。
...
| indexed row | 不接入 | 退化主因仍待消融，不能随 full-cloud 一起分流。 |
...
PI5 结论：generic normal / `float` / full-cloud 已升级为 `production-ready/generic-normal-full-cloud`。它不是整个模板入口 production-ready；`Scalar=double`、indexed 和 correspondences 仍保持标量。
