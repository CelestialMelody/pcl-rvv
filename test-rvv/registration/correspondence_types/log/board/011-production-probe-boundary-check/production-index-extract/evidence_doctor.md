...
- title：correspondence_types board repeated production-probe
- evidence_role：production_direct_probe
- summary_path：log/board/011-production-probe-boundary-check/production-index-extract/summary.md
- comparisons：3
- result：Errors=3，Warnings=1，Suggestions=0

本报告不会自动证明实现有 bug。它用于提示 worker / reviewer：哪些数据模式需要解释、重跑、降级证据边界或补充 metadata。

## Errors（必须修正，否则不能作为 production evidence 或严格性能结论）

### 1. ba_degradation_frequency — match-index production 64K

- observed_pattern：B/A values=1x, 0.918x, 0.983x, 0.907x, 1.02x；3/5 低于 1。
...
### 2. ba_degradation_frequency — query+match production 256K

- observed_pattern：B/A values=0.966x, 0.953x, 0.972x, 0.954x, 0.984x；5/5 低于 1。
- why_suspicious：平均或 median 正向不能掩盖退化频率；高频退化说明候选不稳定或 case 分布被少数值主导。
- possible_non_bug_explanations：可能是测量噪声、温度 / 频率波动、输入局部性变化或 run 数不足。
- possible_bug_or_evidence_issues：也可能是候选在某些点型、规模、row source、mask 或 reduction 路径上真实退化。
- recommended_checks：报告退化频率；必要时扩大 runs，按 point type / size / row source 分组，检查 trace 和 asm attribution。
...
### 3. ba_degradation_frequency — query-index production 4K

- observed_pattern：B/A values=0.877x, 0.729x, 0.863x, 1.12x, 0.895x；4/5 低于 1。
- why_suspicious：平均或 median 正向不能掩盖退化频率；高频退化说明候选不稳定或 case 分布被少数值主导。
- possible_non_bug_explanations：可能是测量噪声、温度 / 频率波动、输入局部性变化或 run 数不足。
- possible_bug_or_evidence_issues：也可能是候选在某些点型、规模、row source、mask 或 reduction 路径上真实退化。
- recommended_checks：报告退化频率；必要时扩大 runs，按 point type / size / row source 分组，检查 trace 和 asm attribution。
...
### 1. long_tail_or_variance — query-index production 4K

- observed_pattern：min=0.729x, median=0.877x, max=1.12x，max/min=1.53。
