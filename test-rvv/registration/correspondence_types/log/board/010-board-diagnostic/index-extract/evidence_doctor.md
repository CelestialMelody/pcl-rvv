...
- title：correspondence_types board repeated diagnostic
- evidence_role：diagnostic
- summary_path：log/board/010-board-diagnostic/index-extract/summary.md
- comparisons：3
...
### 1. ba_degradation_frequency — match-index candidate 64K

- observed_pattern：B/A values=0.983x, 0.989x, 0.934x, 0.933x, 0.959x；5/5 低于 1。
...
### 2. ba_degradation_frequency — query+match candidate 256K

- observed_pattern：B/A values=0.966x, 0.967x, 0.953x, 0.96x, 0.948x；5/5 低于 1。
- why_suspicious：平均或 median 正向不能掩盖退化频率；高频退化说明候选不稳定或 case 分布被少数值主导。
- possible_non_bug_explanations：可能是测量噪声、温度 / 频率波动、输入局部性变化或 run 数不足。
- possible_bug_or_evidence_issues：也可能是候选在某些点型、规模、row source、mask 或 reduction 路径上真实退化。
- recommended_checks：报告退化频率；必要时扩大 runs，按 point type / size / row source 分组，检查 trace 和 asm attribution。
...
### 3. ba_degradation_frequency — query-index candidate 4K

- observed_pattern：B/A values=0.966x, 0.987x, 1.05x, 0.987x, 0.872x；4/5 低于 1。
- why_suspicious：平均或 median 正向不能掩盖退化频率；高频退化说明候选不稳定或 case 分布被少数值主导。
- possible_non_bug_explanations：可能是测量噪声、温度 / 频率波动、输入局部性变化或 run 数不足。
- possible_bug_or_evidence_issues：也可能是候选在某些点型、规模、row source、mask 或 reduction 路径上真实退化。
- recommended_checks：报告退化频率；必要时扩大 runs，按 point type / size / row source 分组，检查 trace 和 asm attribution。
...
### 1. long_tail_or_variance — query-index candidate 4K

- observed_pattern：min=0.872x, median=0.987x, max=1.05x，max/min=1.20。
