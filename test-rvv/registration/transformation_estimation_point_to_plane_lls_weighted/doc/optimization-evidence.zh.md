...
public source-indexed overload
  ...
  -> block-fused-abcd-ilp A/B/C/N groups
  -> staged-gather / compressed-tail rollback if probe helper rejects
  -> solve
```

dual-indices、correspondences、`Scalar=double` 和 layout miss 路径保持标量。

这里的 source-indexed 当前状态是 Phase 031 的 bounded production candidate（有边界的生产候选）：真实 public dispatch 先尝试 `block-fused-abcd-ilp`，但 source-indexed-specific asm、binary identity 和长尾解释仍未闭合，还不能写成 clean adopted。Phase 030 的 source-indexed family repeated diagnostic 已降级为 historical diagnostic / harness-risk signal（历史诊断 / 测试框架风险信号）；dual-indices / correspondences 仍为 diagnostic negative。不能把 full-cloud positive 或 source-indexed probe positive 直接外推成其它入口的最优结论。
...
| source-indexed RVV dispatch 与 block-fused probe gate | bounded production candidate | `estimateRigidTransformation(cloud_src, indices_src, cloud_tgt, matrix)`、`estimatePointToPlaneLLSWeightedSourceIndicesRVV`、`buildPointToPlaneLLSWeightedSourceIndicesBlockFusedAbcdIlpRVV`、`buildPointToPlaneLLSWeightedSourceIndicesStagedRVV`、`buildPointToPlaneLLSWeightedSourceIndicesDefault`、`canUsePointToPlaneLLSWeightedSourceIndicesRVV` | `run_test_public_semantics`、`run_test_input_semantics`、`run_test_production_direct`、`run_test_source_indices`、`run_board_test_source_indices` | `run_bench_production_source_indices`、`run_board_bench_production_source_indices`、`collect_board_production_source_indices_probe_repeated`、`doctor_board_production_source_indices_probe_repeated` | 真实 public source-indexed 入口能在有效 source index、连续 weights 和 generic layout gate 时走 block-fused probe RVV；fallback 保持 staged-gather / 标量语义。 | dual-indices、correspondences、非法 index public API 行为；也不能证明 clean adopted，因为 asm / binary identity / taskset metadata 仍缺。 |
| source-indexed staged-gather / compressed-tail rollback | historical production baseline / rollback helper | `buildPointToPlaneLLSWeightedSourceIndicesStagedRVV`、`loadPointToPlaneLLSWeightedSourceIndexedVectors`、`accumulatePointToPlaneLLSWeightedCompressedRowsF32M2` | `run_test_source_indices` | `collect_board_production_source_indices_repeated`、`production_source_indices_staged_gather/summary.md` | 保留 Phase 031 前真实 public source-indexed 入口的正向 production baseline。 | 不再是当前优先 dispatch；不解释 Phase 030 / 031 分歧。 |
...
| 取数 | 逐点读 source、target、weight。 | full-cloud 读 source / target AoS 字段并连续读 weights；source-indexed block-fused probe 额外先把索引展开成 `uint32_t` staging，再按 source gather / target stride load 取数。 | `run_test_production_direct`、`run_bench_production_dispatch`、`run_test_source_indices`、`run_bench_production_source_indices`。 |
...
| 公式 | 逐点计算 `a/b/c/d`。 | full-cloud 在 `loadPointToPlaneLLSWeightedFullReductionVectors` 中生成向量 `a/b/c/d`；source-indexed block-fused probe 在 `loadPointToPlaneLLSWeightedSourceIndexedBlockVectors` 中生成同类向量。 | `run_test_candidates`、`run_test_source_indices`。 |
| 规约 | 逐点累加到 normal-equation。 | full-cloud 和 source-indexed block-fused probe 都使用 A/B/C/N block groups 维护向量 partial sums，再显式 `vfredosum`；source-indexed staged-gather rollback 仍使用 `vcompress` 后 scalar tail accumulation。 | `run_test_candidates`、`run_test_source_indices`、`trace_summary.md`、probe summary。 |
...
| `estimateRigidTransformation(cloud_src, indices_src, cloud_tgt, matrix)` | 真实 public source-indexed 入口；只在有效 source index 和 gate 成立时尝试 RVV。 | `run_test_source_indices`、`run_test_input_semantics`、`run_test_public_semantics` | `run_test_source_indices_std.log`、`run_test_source_indices_rvv.log`、`production_source_indices_block_fused_abcd_ilp_probe/summary.md` |
...
| `estimatePointToPlaneLLSWeightedSourceIndicesRVV(...)` | source-indexed RVV wrapper；先尝试 block-fused probe，再尝试 staged-gather rollback。 | `run_test_source_indices`、`run_bench_production_source_indices` | `run_test_source_indices_rvv.log`、`production_source_indices_block_fused_abcd_ilp_probe/summary.md` |
| `buildPointToPlaneLLSWeightedSourceIndicesBlockFusedAbcdIlpRVV(...)` | valid-index scan、`uint32_t` staging、source gather、target stride load、weight load、A/B/C/N block groups。 | `run_test_source_indices`、`run_test_production_direct`、`run_bench_production_source_indices` | `production_source_indices_block_fused_abcd_ilp_probe/summary.md`、`evidence_doctor.md` |
| `buildPointToPlaneLLSWeightedSourceIndicesStagedRVV(...)` | valid-index scan、`uint32_t` staging、source gather、target stride load、weight load、`vcompress` 和 scalar tail；Phase 031 后作为 rollback helper。 | `run_test_source_indices`、`run_bench_production_source_indices` | `production_source_indices_staged_gather/summary.md` |
| `buildPointToPlaneLLSWeightedSourceIndicesDefault(...)` | source-indexed block-fused / staged-gather RVV 都 miss 时回标量。 | `run_test_source_indices` | `run_test_source_indices_std.log`、`run_test_source_indices_rvv.log` |
...
| source-indexed production helper | bounded production candidate | `run_test_source_indices`、`run_bench_production_source_indices`、`run_board_bench_production_source_indices`、`collect_board_production_source_indices_probe_repeated` | 已有 production direct、fallback、代表点型 probe board summary 和 dedicated bench；clean adopted 仍缺 asm / binary identity / taskset metadata。 |
...
source-indexed 先由 `log/board/run_board_bench_row_sources/analyze_bench_compare.log` 的 row-source 单次板卡诊断触发 production integration loop。Phase 031 进一步把真实 public source-indexed overload 接到 block-fused probe，并用 dedicated production target 复核，不来自默认综合 bench。当前 probe summary 路径是：
...
该结果只支持 valid source-indexed public overload 上的 block-fused probe。它不支持 dual-indices、correspondences、invalid index public API 行为、`Scalar=double` 或非连续权重。Doctor 为 0 Errors / 9 Warnings / 12 Suggestions，因此当前只写 bounded production candidate。Phase 031 前的 staged-gather summary `production_source_indices_staged_gather/summary.md` 保留为 rollback baseline。
...
estimateRigidTransformation(cloud_src, indices_src, cloud_tgt, matrix)
  ...
  -> buildPointToPlaneLLSWeightedSourceIndicesBlockFusedAbcdIlpRVV(...)
     ...
     else buildPointToPlaneLLSWeightedSourceIndicesStagedRVV(...)
       -> source gather + target stride load + vcompress + scalar tail
  -> solvePointToPlaneLLSWeightedNormalEquation(...)
```

这条链路说明当前采用的是 block-fused probe 优先、staged-gather rollback 保留的形态。它不是把 Phase 030 diagnostic wrapper 结果直接写成生产结论，而是在真实 public dispatch 上重新验证。

| 维度 | 当前状态 | 当前证据 | 不能外推 |
| --- | --- | --- | --- |
| source-indexed block-fused-abcd-ilp production probe | bounded production candidate | `run_test_source_indices_compare`、`run_test_production_direct_compare`、`collect_board_production_source_indices_probe_repeated`、`production_source_indices_block_fused_abcd_ilp_probe/summary.md`、`evidence_manifest.json`、`evidence_doctor.md` | clean adopted；source-indexed-specific asm、binary identity、taskset metadata、extended-run。 |
| source-indexed staged-gather / compressed-tail | historical production baseline / rollback | `run_test_source_indices_compare`、`run_bench_production_source_indices`、`collect_board_production_source_indices_repeated`、`production_source_indices_staged_gather/summary.md` | 当前 probe 的 clean adoption，也不解释 Phase 030 / 031 分歧。 |
...
- source-indexed public overload 的 RVV dispatch 可以在有效 source index、连续权重和当前 gate 下命中 block-fused production probe。
- `Scalar=float`、连续 `weights_`、source xyz f32 AoS、target xyz+normal f32 AoS 的路径已经有 correctness、trace、checksum 和 asm 归因证据。
- source-indexed block-fused probe 路径已经有 correctness、bench 和 repeated board speedup 证据，但还缺 clean adopted 所需的 asm / binary identity / taskset metadata。
