...
enum class WeightedFusedFormulaMode {
  AbcFused,
  AbcFusedIlp,
  DSixTermFma,
  DSixTermFmaIlp,
  DDisplacementFused,
  DDisplacementFusedIlp,
  AbcdFused,
  AbcdFusedIlp,
  BlockBaseline,
};
...
inline void
weighted_formula_block_baseline_m1(vfloat32m1_t sx,
                                   vfloat32m1_t sy,
                                   vfloat32m1_t sz,
                                   vfloat32m1_t dx,
                                   vfloat32m1_t dy,
                                   vfloat32m1_t dz,
                                   vfloat32m1_t normal_x,
                                   vfloat32m1_t normal_y,
                                   vfloat32m1_t normal_z,
                                   vfloat32m1_t weight,
                                   const std::size_t vl,
                                   vfloat32m1_t& a,
                                   vfloat32m1_t& b,
...
inline void
weighted_formula_abc_fused_m1(vfloat32m1_t sx,
                              vfloat32m1_t sy,
                              vfloat32m1_t sz,
                              vfloat32m1_t dx,
                              vfloat32m1_t dy,
                              ...
                              vfloat32m1_t& nz)
{
  // abc-fused：只把 a/b/c 迁移成 unweighted TEPTPL 同款 vfmsac 形态；
  // d 保持 current six-term 展开，便于把旋转项和位移项的数值影响分开看。
  nx = __riscv_vfmul_vv_f32m1(normal_x, weight, vl);
  ny = __riscv_vfmul_vv_f32m1(normal_y, weight, vl);
  nz = __riscv_vfmul_vv_f32m1(normal_z, weight, vl);

  // This vfmsac operand order is validated against the scalar normal equation.
  // Reordering by visual sign inspection flips the solved rotation terms.
...
inline void
weighted_formula_abc_fused_ilp_m1(vfloat32m1_t sx,
                                  vfloat32m1_t sy,
                                  vfloat32m1_t sz,
                                  vfloat32m1_t dx,
                                  vfloat32m1_t dy,
                                  ...
                                  vfloat32m1_t& nz)
{
  // abc-fused-ilp：只重排 a/b/c 三条互相独立的 seed multiply，再执行对应
  // vfmsac；d 仍保持 current six-term 展开，用来单独观察调度/RAW 形状。
  nx = __riscv_vfmul_vv_f32m1(normal_x, weight, vl);
  ny = __riscv_vfmul_vv_f32m1(normal_y, weight, vl);
  nz = __riscv_vfmul_vv_f32m1(normal_z, weight, vl);
...
inline void
weighted_formula_d_six_term_fma_m1(vfloat32m1_t sx,
                                   vfloat32m1_t sy,
                                   vfloat32m1_t sz,
                                   vfloat32m1_t dx,
                                   vfloat32m1_t dy,
                                   ...
                                   vfloat32m1_t& nz)
{
  // d-six-term-fma：保留 target 三项减 source 三项的语义分组，只把每个三项和
  // 改成 vfmacc 累加。它能隔离“d 的乘加收缩”而不引入 displacement 公式树。
  nx = __riscv_vfmul_vv_f32m1(normal_x, weight, vl);
  ny = __riscv_vfmul_vv_f32m1(normal_y, weight, vl);
  nz = __riscv_vfmul_vv_f32m1(normal_z, weight, vl);
  ...
  vfloat32m1_t target_sum = __riscv_vfmul_vv_f32m1(nx, dx, vl);
  target_sum = __riscv_vfmacc_vv_f32m1(target_sum, ny, dy, vl);
  target_sum = __riscv_vfmacc_vv_f32m1(target_sum, nz, dz, vl);
  vfloat32m1_t source_sum = __riscv_vfmul_vv_f32m1(nx, sx, vl);
  source_sum = __riscv_vfmacc_vv_f32m1(source_sum, ny, sy, vl);
  source_sum = __riscv_vfmacc_vv_f32m1(source_sum, nz, sz, vl);
  d = __riscv_vfsub_vv_f32m1(target_sum, source_sum, vl);
}

inline void
weighted_formula_d_six_term_fma_ilp_m1(vfloat32m1_t sx,
                                       vfloat32m1_t sy,
                                       vfloat32m1_t sz,
                                       vfloat32m1_t dx,
                                       vfloat32m1_t dy,
                                       ...
                                       vfloat32m1_t& nz)
{
  // d-six-term-fma-ilp：公式与 d-six-term-fma 相同，只把互相独立的
  // abc seed/sub 和 target/source d accumulator 交错，观察调度/codegen 形态。
  nx = __riscv_vfmul_vv_f32m1(normal_x, weight, vl);
  ny = __riscv_vfmul_vv_f32m1(normal_y, weight, vl);
  nz = __riscv_vfmul_vv_f32m1(normal_z, weight, vl);

  const vfloat32m1_t a_lhs = __riscv_vfmul_vv_f32m1(nz, sy, vl);
  const vfloat32m1_t b_lhs = __riscv_vfmul_vv_f32m1(nx, sz, vl);
  const vfloat32m1_t c_lhs = __riscv_vfmul_vv_f32m1(ny, sx, vl);
  const vfloat32m1_t a_rhs = __riscv_vfmul_vv_f32m1(ny, sz, vl);
  const vfloat32m1_t b_rhs = __riscv_vfmul_vv_f32m1(nz, sx, vl);
  const vfloat32m1_t c_rhs = __riscv_vfmul_vv_f32m1(nx, sy, vl);
  ...
  a = __riscv_vfsub_vv_f32m1(a_lhs, a_rhs, vl);
  b = __riscv_vfsub_vv_f32m1(b_lhs, b_rhs, vl);
  c = __riscv_vfsub_vv_f32m1(c_lhs, c_rhs, vl);
  d = __riscv_vfsub_vv_f32m1(target_sum, source_sum, vl);
}

inline void
weighted_formula_d_displacement_fused_m1(vfloat32m1_t sx,
                                         vfloat32m1_t sy,
                                         vfloat32m1_t sz,
                                         vfloat32m1_t dx,
                                         vfloat32m1_t dy,
                                         ...
                                         vfloat32m1_t& nz)
{
  // d-displacement-fused：a/b/c 保持 current 公式；d 改成
  // nx*(dx-sx) + ny*(dy-sy) + nz*(dz-sz)。这更接近 unweighted fused 经验，
  // 但抵消发生在位移差上，所以必须由 near-cancellation 和 scale-stress 保护。
  nx = __riscv_vfmul_vv_f32m1(normal_x, weight, vl);
  ny = __riscv_vfmul_vv_f32m1(normal_y, weight, vl);
  nz = __riscv_vfmul_vv_f32m1(normal_z, weight, vl);
...
inline void
weighted_formula_d_displacement_fused_ilp_m1(vfloat32m1_t sx,
                                             vfloat32m1_t sy,
                                             vfloat32m1_t sz,
                                             vfloat32m1_t dx,
                                             vfloat32m1_t dy,
                                             ...
                                             vfloat32m1_t& nz)
{
  // d-displacement-fused-ilp：公式与 displacement 形态相同，先暴露
  // abc 左右项和 dx-sx/dy-sy/dz-sz 的独立性，再做 d 的短 FMA 链。
  nx = __riscv_vfmul_vv_f32m1(normal_x, weight, vl);
  ny = __riscv_vfmul_vv_f32m1(normal_y, weight, vl);
  nz = __riscv_vfmul_vv_f32m1(normal_z, weight, vl);
...
inline void
weighted_formula_abcd_fused_m1(vfloat32m1_t sx,
                               vfloat32m1_t sy,
                               vfloat32m1_t sz,
                               vfloat32m1_t dx,
                               vfloat32m1_t dy,
                               ...
                               vfloat32m1_t& nz)
{
  // abcd-fused：组合 abc-fused 与 d-displacement-fused，只作为 isolated
  // candidates 都通过后的诊断组合，不自动代表 production 可接入。
  nx = __riscv_vfmul_vv_f32m1(normal_x, weight, vl);
  ny = __riscv_vfmul_vv_f32m1(normal_y, weight, vl);
  nz = __riscv_vfmul_vv_f32m1(normal_z, weight, vl);
...
inline void
weighted_formula_abcd_fused_ilp_m1(vfloat32m1_t sx,
                                   vfloat32m1_t sy,
                                   vfloat32m1_t sz,
                                   vfloat32m1_t dx,
                                   vfloat32m1_t dy,
                                   ...
                                   vfloat32m1_t& nz)
{
  // abcd-fused-ilp：组合 abc-fused-ilp 和 d-displacement-fused-ilp。
  // 它只用于诊断组合收益，不能跳过 isolated candidates 直接接 production。
  nx = __riscv_vfmul_vv_f32m1(normal_x, weight, vl);
  ny = __riscv_vfmul_vv_f32m1(normal_y, weight, vl);
  nz = __riscv_vfmul_vv_f32m1(normal_z, weight, vl);
...
inline void
weighted_fused_formula_m1(vfloat32m1_t sx,
                          vfloat32m1_t sy,
                          vfloat32m1_t sz,
                          vfloat32m1_t dx,
                          vfloat32m1_t dy,
...
template <>
inline void
weighted_fused_formula_m1<WeightedFusedFormulaMode::AbcFused>(
    vfloat32m1_t sx,
    vfloat32m1_t sy,
    vfloat32m1_t sz,
    vfloat32m1_t dx,
    ...
    vfloat32m1_t& nz)
{
  weighted_formula_abc_fused_m1(sx, sy, sz, dx, dy, dz, normal_x, normal_y,
                                normal_z, weight, vl, a, b, c, d, nx, ny, nz);
}

template <>
inline void
weighted_fused_formula_m1<WeightedFusedFormulaMode::AbcFusedIlp>(
    vfloat32m1_t sx,
    vfloat32m1_t sy,
    vfloat32m1_t sz,
    vfloat32m1_t dx,
    ...
    vfloat32m1_t& nz)
{
  weighted_formula_abc_fused_ilp_m1(sx,
                                    sy,
                                    sz,
                                    dx,
                                    dy,
...
template <>
inline void
weighted_fused_formula_m1<WeightedFusedFormulaMode::DSixTermFma>(
    vfloat32m1_t sx,
    vfloat32m1_t sy,
    vfloat32m1_t sz,
    vfloat32m1_t dx,
    ...
    vfloat32m1_t& nz)
{
  weighted_formula_d_six_term_fma_m1(sx, sy, sz, dx, dy, dz, normal_x,
                                     normal_y, normal_z, weight, vl, a, b, c,
                                     d, nx, ny, nz);
}

template <>
inline void
weighted_fused_formula_m1<WeightedFusedFormulaMode::DSixTermFmaIlp>(
    vfloat32m1_t sx,
    vfloat32m1_t sy,
    vfloat32m1_t sz,
    vfloat32m1_t dx,
    ...
    vfloat32m1_t& nz)
{
  weighted_formula_d_six_term_fma_ilp_m1(sx, sy, sz, dx, dy, dz, normal_x,
                                         normal_y, normal_z, weight, vl, a, b,
                                         c, d, nx, ny, nz);
}

template <>
inline void
weighted_fused_formula_m1<WeightedFusedFormulaMode::DDisplacementFused>(
    vfloat32m1_t sx,
    vfloat32m1_t sy,
    vfloat32m1_t sz,
    vfloat32m1_t dx,
    ...
    vfloat32m1_t& nz)
{
  weighted_formula_d_displacement_fused_m1(sx, sy, sz, dx, dy, dz, normal_x,
                                           normal_y, normal_z, weight, vl, a,
                                           b, c, d, nx, ny, nz);
}

template <>
inline void
weighted_fused_formula_m1<WeightedFusedFormulaMode::DDisplacementFusedIlp>(
    vfloat32m1_t sx,
    vfloat32m1_t sy,
    vfloat32m1_t sz,
    vfloat32m1_t dx,
    ...
    vfloat32m1_t& nz)
{
  weighted_formula_d_displacement_fused_ilp_m1(
      sx, sy, sz, dx, dy, dz, normal_x, normal_y, normal_z, weight, vl, a, b, c,
      d, nx, ny, nz);
}

template <>
inline void
weighted_fused_formula_m1<WeightedFusedFormulaMode::AbcdFused>(
    vfloat32m1_t sx,
    vfloat32m1_t sy,
    vfloat32m1_t sz,
    vfloat32m1_t dx,
    ...
    vfloat32m1_t& nz)
{
  weighted_formula_abcd_fused_m1(sx, sy, sz, dx, dy, dz, normal_x, normal_y,
                                 normal_z, weight, vl, a, b, c, d, nx, ny, nz);
}

template <>
inline void
weighted_fused_formula_m1<WeightedFusedFormulaMode::AbcdFusedIlp>(
    vfloat32m1_t sx,
    vfloat32m1_t sy,
    vfloat32m1_t sz,
    vfloat32m1_t dx,
    ...
    vfloat32m1_t& nz)
{
  weighted_formula_abcd_fused_ilp_m1(sx, sy, sz, dx, dy, dz, normal_x, normal_y,
                                     normal_z, weight, vl, a, b, c, d, nx, ny,
                                     nz);
}

template <>
inline void
weighted_fused_formula_m1<WeightedFusedFormulaMode::BlockBaseline>(
    vfloat32m1_t sx,
    vfloat32m1_t sy,
    vfloat32m1_t sz,
    vfloat32m1_t dx,
    ...
    vfloat32m1_t& nz)
{
  weighted_formula_block_baseline_m1(sx,
                                     sy,
                                     sz,
                                     dx,
                                     dy,
...
template <WeightedFusedFormulaMode Mode,
          typename PointSource = pcl::PointNormal,
          typename PointTarget = pcl::PointNormal,
          typename SrcLayout = pcl::rvv::RVVXYZAoSFloatLayout<PointSource>,
          typename TgtLayout = pcl::rvv::RVVXYZNormalFloatLayout<PointTarget>>
inline void
load_full_block_reduction_fused_vectors(const std::uint8_t* source_base,
                                        const std::uint8_t* target_base,
                                        const float* weights,
                                        const std::size_t i,
                                        const std::size_t vl,
                                            ...
                                            NormalEquation& eq)
{
  ...
  for (std::size_t i = begin; i < end;) {
    ...
    vfloat32m1_t a, b, c, d, nx, ny, nz;
    vbool32_t keep;
    load_full_block_reduction_fused_vectors<Mode,
                                             PointSource,
                                             PointTarget,
                                             SrcLayout,
                                             TgtLayout>(
