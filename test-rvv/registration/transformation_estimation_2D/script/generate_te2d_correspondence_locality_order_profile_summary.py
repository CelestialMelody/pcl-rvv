#!/usr/bin/env python3
"""生成 transformation_estimation_2D correspondence locality/order profile 摘要。"""
...
LOCALITY_RE = re.compile(
    r"^(?P<kind>direct correspondence public|staged-dual-indexed public|materialize-ordered public) "
    r"2D correspondence-pair (?P<profile>[A-Za-z0-9_-]+) (?P<size>\d+K)$"
)

KIND_TO_KEY = {
    "direct correspondence public": "direct",
    "staged-dual-indexed public": "staged",
    "materialize-ordered public": "materialize",
}
...
def parse_case_name(name: str) -> tuple[str, str, str] | None:
  match = LOCALITY_RE.match(name)
  if not match:
    return None
  return KIND_TO_KEY[match.group("kind")], match.group("profile"), match.group("size")
...
def build_metrics(runs: list[dict[str, Any]]) -> dict[tuple[str, str], dict[str, Any]]:
  ...
  if not expected:
    raise SystemExit("no correspondence locality/order profile cases found")

  metrics: dict[tuple[str, str], dict[str, Any]] = {}
  for profile, size in sorted(expected, key=lambda item: (item[0], size_value(item[1]))):
    labels = {
        "direct": f"direct correspondence public 2D correspondence-pair {profile} {size}",
        "staged": f"staged-dual-indexed public 2D correspondence-pair {profile} {size}",
        "materialize": f"materialize-ordered public 2D correspondence-pair {profile} {size}",
    }
    ...
    for run in runs:
      ...
      direct = cases[labels["direct"]]
      staged = cases[labels["staged"]]
      materialize = cases[labels["materialize"]]
...
def overall_decision(metrics: dict[tuple[str, str], dict[str, Any]]) -> str:
  target = [
      info
      for (_, size), info in metrics.items()
      if size in ("64K", "256K")
...
def side(boundary: str,
  ...
  return {
      ...
      "checksum": checksum,
      "timer_boundary": "correspondence_locality_order_profile_ingress_plus_two_pass_centered_correlation_plus_2d_solve",
...
def build_manifest(
  ...
  for (profile, size), info in metrics.items():
    ...
    comparisons.append(
        {
            "name": f"correspondence locality/order direct-vs-staged profile {profile} {size}",
            "group": "te2d_correspondence_locality_order_profile",
            "case_kind": "production_detail_locality_order_profile",
            ...
            "notes": (
                ...
                "大于 1 表示 staged-dual 更快。This is a locality/order profile, not adoption evidence."
            ),
            "baseline": side(
  ...
  return {
      "schema_version": 1,
      "summary": {
          "title": "transformation_estimation_2D correspondence locality/order profile",
          "evidence_role": "production_detail",
          "summary_path": str(summary_path),
          "label": "correspondence direct public vs staged-dual public by locality/order",
          "analysis_script": "test-rvv/registration/transformation_estimation_2D/script/generate_te2d_correspondence_locality_order_profile_summary.py",
          "metadata": {
...
def render_summary(
  ...
  first_meta = runs[0]["rvv"]["metadata"]
  lines: list[str] = [
      "# transformation_estimation_2D Correspondence Locality/Order Profile Summary",
      "",
      "## 运行合同",
      "",
      ...
      "- evidence_role：`production_detail_profile`；这是 locality/order profile，不是 production adoption。",
      ...
      "- M/D：`materialize ordered public RVV ms / direct correspondence public RVV ms`，大于 1 表示 direct 快于完整物化。",
      ...
      "## 结果",
      "",
      "| profile | size | D/S values | D/S median | D/S<1 | bucket | M/D median | M/S median | direct_ms | staged_ms | materialize_ms | checksum |",
      "| --- | --- | --- | ---: | ---: | --- | ---: | ---: | ---: | ---: | ---: | --- |",
  ]
  for (profile, size), info in metrics.items():
    ...
    lines.append(
        ...
        f"`{info['bucket']}` | {statistics.median(info['materialize_over_direct']):.3f}x | "
        f"{statistics.median(info['materialize_over_staged']):.3f}x | "
  ...
  lines.extend(
      [
          ...
          "- 本 summary 只能解释 locality/order 对 direct、staged 和 materialize 三条路径的影响。",
          "- 任何正向 profile 都必须另开 production PI2-PI5，不能在本阶段切换 dispatch。",
          "",
      ]
  )
  return "\n".join(lines)
...
def main() -> int:
  ...
  parser.add_argument("--case-filter", default="correspondence-locality-order-profile")
