...
| `PublicSourceIndexedPointTypeOutsidePhaseScopeUsesScalarFallback` | `PointXYZI -> PointXYZI` source-indexed 有效输入 | 真实 source-indexed-cloud-pair 公开入口 | 输出矩阵与 double public scalar reference 对齐。 | Phase 091 不覆盖 source-indexed generic PointXYZ-like 点型；Phase 099 仍未接 production dispatch。 |
