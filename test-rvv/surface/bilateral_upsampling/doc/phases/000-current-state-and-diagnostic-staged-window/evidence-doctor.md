...
- title：surface/bilateral_upsampling staged-window-reduction board diagnostic
...
### 1. ba_degradation_frequency — bilateral upsampling table window 80x60 w3 dense

- observed_pattern：B/A values=0.9x；1/1 低于 1。
...
### 2. ba_degradation_frequency — bilateral upsampling table window 120x90 w4 holes
...
### 3. ba_degradation_frequency — bilateral upsampling table window 180x120 w5 dense
...
### 1. contract_mismatch — bilateral upsampling table window 80x60 w3 dense

- observed_pattern：字段 'wrapper' 不一致：baseline='processScalar', candidate='processCandidate'。
...
### 2. contract_mismatch — bilateral upsampling table window 80x60 w3 dense

- observed_pattern：字段 'timer_boundary' 不一致：baseline='window_scan_table_lookup_reduction_unprojection', candidate='window_scan_table_lookup_staging_rvv_reduction_unprojection'。
- why_suspicious：A/B 两侧边界不一致时，数值差异可能来自 wrapper、row source、solve 或计时口径，而不是候选本身。
- possible_non_bug_explanations：可能这是有意设计的 cross-check，用于了解真实 public overload 与 helper 的差异。
- possible_bug_or_evidence_issues：也可能是表名或 summary 角色错误，把 mixed-boundary 数据当成 strict A/B。
- recommended_checks：若有意混合边界，重命名为 mixed-boundary cross-check 并降级结论；若不是，按同边界重跑。
...
### 3. contract_mismatch — bilateral upsampling table window 80x60 w3 dense
...
### 4. contract_mismatch — bilateral upsampling table window 120x90 w4 holes
...
### 5. contract_mismatch — bilateral upsampling table window 120x90 w4 holes
...
### 6. contract_mismatch — bilateral upsampling table window 120x90 w4 holes
...
### 7. contract_mismatch — bilateral upsampling table window 180x120 w5 dense
...
### 8. contract_mismatch — bilateral upsampling table window 180x120 w5 dense
...
### 9. contract_mismatch — bilateral upsampling table window 180x120 w5 dense
