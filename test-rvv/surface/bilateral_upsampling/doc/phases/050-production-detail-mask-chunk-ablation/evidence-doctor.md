...
- title：surface/bilateral_upsampling production detail mask/chunk ablation
...
- comparisons：3
- result：Errors=2，Warnings=4，Suggestions=1

本报告不会自动证明实现有 bug。它用于提示 worker / reviewer：哪些数据模式需要解释、重跑、降级证据边界或补充 metadata。

## Errors（必须修正，否则不能作为 production evidence 或严格性能结论）

### 1. ba_degradation_frequency — bilateral upsampling production detail local nan-mask k64 PointXYZRGB 80x60 w3 dense
...
### 2. ba_degradation_frequency — bilateral upsampling production detail local nan-mask k64 PointXYZRGB 120x90 w4 holes

- observed_pattern：B/A values=0.92x；1/1 低于 1。
- why_suspicious：平均或 median 正向不能掩盖退化频率；高频退化说明候选不稳定或 case 分布被少数值主导。
- possible_non_bug_explanations：可能是测量噪声、温度 / 频率波动、输入局部性变化或 run 数不足。
- possible_bug_or_evidence_issues：也可能是候选在某些点型、规模、row source、mask 或 reduction 路径上真实退化。
- recommended_checks：报告退化频率；必要时扩大 runs，按 point type / size / row source 分组，检查 trace 和 asm attribution。
...
### 1. contract_mismatch — bilateral upsampling production detail local nan-mask k64 PointXYZRGB 80x60 w3 dense

- observed_pattern：字段 'mask' 不一致：baseline='finite_depth_skip_with_vmfeq_merge', candidate='finite_depth_nan_only_k64'。
...
### 2. contract_mismatch — bilateral upsampling production detail local nan-mask k64 PointXYZRGB 120x90 w4 holes
...
### 3. contract_mismatch — bilateral upsampling production detail local nan-mask k64 PointXYZRGBA 180x120 w5 dense
...
### 4. group_outlier — bilateral upsampling production detail local nan-mask k64 PointXYZRGBA 180x120 w5 dense

- observed_pattern：group=production-detail-mask-chunk-ablation 的组内 median=0.98x；当前 case median=1.01x，偏离 3.1%。
...
### 1. near_threshold_ba — bilateral upsampling production detail local nan-mask k64 PointXYZRGBA 180x120 w5 dense
