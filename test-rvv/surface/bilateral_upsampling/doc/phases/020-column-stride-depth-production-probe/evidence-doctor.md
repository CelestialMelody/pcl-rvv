...
- title：surface/bilateral_upsampling column-stride-depth production public probe
...
### 1. ba_degradation_frequency — bilateral upsampling production public PointXYZRGB 80x60 w3 dense

- observed_pattern：B/A values=0.96x；1/1 低于 1。
...
### 2. ba_degradation_frequency — bilateral upsampling production public PointXYZRGB 120x90 w4 holes

- observed_pattern：B/A values=0.89x；1/1 低于 1。
- why_suspicious：平均或 median 正向不能掩盖退化频率；高频退化说明候选不稳定或 case 分布被少数值主导。
- possible_non_bug_explanations：可能是测量噪声、温度 / 频率波动、输入局部性变化或 run 数不足。
- possible_bug_or_evidence_issues：也可能是候选在某些点型、规模、row source、mask 或 reduction 路径上真实退化。
- recommended_checks：报告退化频率；必要时扩大 runs，按 point type / size / row source 分组，检查 trace 和 asm attribution。
...
### 3. ba_degradation_frequency — bilateral upsampling production public PointXYZRGBA 180x120 w5 dense

- observed_pattern：B/A values=0.95x；1/1 低于 1。
- why_suspicious：平均或 median 正向不能掩盖退化频率；高频退化说明候选不稳定或 case 分布被少数值主导。
- possible_non_bug_explanations：可能是测量噪声、温度 / 频率波动、输入局部性变化或 run 数不足。
- possible_bug_or_evidence_issues：也可能是候选在某些点型、规模、row source、mask 或 reduction 路径上真实退化。
- recommended_checks：报告退化频率；必要时扩大 runs，按 point type / size / row source 分组，检查 trace 和 asm attribution。
...
### 1. contract_mismatch — bilateral upsampling production public PointXYZRGB 80x60 w3 dense
...
### 2. contract_mismatch — bilateral upsampling production public PointXYZRGB 80x60 w3 dense
...
### 3. contract_mismatch — bilateral upsampling production public PointXYZRGB 80x60 w3 dense
...
### 4. contract_mismatch — bilateral upsampling production public PointXYZRGB 120x90 w4 holes
...
### 5. contract_mismatch — bilateral upsampling production public PointXYZRGB 120x90 w4 holes
...
### 6. contract_mismatch — bilateral upsampling production public PointXYZRGB 120x90 w4 holes
...
### 7. contract_mismatch — bilateral upsampling production public PointXYZRGBA 180x120 w5 dense
...
### 8. contract_mismatch — bilateral upsampling production public PointXYZRGBA 180x120 w5 dense
...
### 9. contract_mismatch — bilateral upsampling production public PointXYZRGBA 180x120 w5 dense
