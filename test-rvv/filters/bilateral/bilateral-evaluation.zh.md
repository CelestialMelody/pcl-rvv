...
主题来自 `filters-module-followup-rescreen.zh.md` 的 `6.3` 观察项，本轮按诊断候选提升后完成函数级评估，并在补充误差预算后接入生产 `PointXYZI` 主路径。
...
| --- | --- | --- | --- |
| `BilateralFilter<PointXYZI>::computePointWeight` | 接入生产 RVV helper | `__RVV10__`、`PointXYZI`、邻域数 `>=16`、`indices.size()==distances.size()` | 小邻域、非 `PointXYZI`、非 RVV 编译、异常邻域长度回退 `computePointWeightStd` |
| 邻域 intensity 读取 | RVV gather | `radiusSearch` 返回非负 point id，`PointXYZI::intensity` 布局固定 | 非标准 intensity 字段不覆盖 |
...
生产 helper 放在 `filters/include/pcl/filters/impl/bilateral.hpp` 的入口附近，不改变公开 API。`computePointWeightStd` 常驻为标量 helper，`computePointWeightRVV` 仅在 `__RVV10__` 下定义，公开成员 `computePointWeight` 按类型和宏短路分流。
...
- `gather_load_f32m2<PointT, offsetof(PointT, intensity)>` gather `PointXYZI::intensity`；
...
上游原始测试复用 `test/filters/test_bilateral.cpp` 的 `FastBilateralFilter.Filters_Bilateral`，用于确认 filters bilateral 相关上游测试仍可运行；`BilateralFilter<PointXYZI>` 本体由专项测试覆盖。
...
- 反汇编：`output/qemu/rvv_asm_check.log` 确认 `applyFilter` 调用 `computePointWeightRVV<PointXYZI>`，并命中 `vsetvli`、`vle32.v`、`vluxei32.v`、`vfsub.vv`、`vfmul.vv`、`vfcvt.*.v`、`vsll.vi`、`vfmacc.vv`、`vse32.v` 等路径。
...
结论：误差远低于专项预算 `max_abs <= 8e-4`、`max_rel <= 5e-5`、`rmse <= 2e-4`，且板卡生产入口收益明显。`bilateral` 从 6.3 观察项升级为 `PointXYZI` 生产 RVV 接入完成；非覆盖类型和短邻域保持标量 fallback。
