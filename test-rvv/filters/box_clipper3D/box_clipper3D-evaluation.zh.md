...
- 非 `PointXYZ` 泛型点类型：避免泛型 `getVector4fMap()`、继承布局和第四齐次分量假设扩大；
...
- 公开入口用 `if constexpr (std::is_same_v<PointT, pcl::PointXYZ>)` 短路调用 RVV，否则落回 Std；
...
| 第四齐次分量 | `PointXYZ` 的 `PCL_ADD_POINT4D` 保证 `getVector4fMap()` 对应 `[x,y,z,1]`；非 `PointXYZ` 保持标量 |
...
- `Dataset: synthetic PointXYZ clouds; full-cloud affine box clipping RVV cases and subset/type fallback cases`
- `Iterations: 30`
...
| `box_clipper3D pointxyzi fallback 1M` | `BoxClipper3D<PointXYZI>` | 1M `PointXYZI` | 类型 fallback 语义 / 成本 |
...
- QEMU bench：`make -C test-rvv/filters/box_clipper3D run_bench_compare` 通过，std/RVV checksum 对齐；`output/qemu/analyze_bench_compare.log` 无 `未解析`、`n/a`、`Total Time 不计算`。QEMU 主路径 RVV 慢于 Std，只记录为模拟器执行现象，不作为性能结论。
...
设备：`Milkv-Jupiter`。数据集：synthetic `PointXYZ` clouds，full-cloud affine box clipping RVV cases and subset/type fallback cases。Iterations: `30`。
...
| `box_clipper3D pointxyzi fallback 1M` | 61.7393 | 62.3824 | 0.99x | 非 `PointXYZ` 类型 fallback，不作为 RVV 主路径性能结论 |

结论：`BoxClipper3D<PointXYZ>::clipPointCloud3D` 全云主路径在板卡上约 `4.95x` 到 `5.86x`。subset 与非 `PointXYZ` fallback 不作为 RVV 性能结论。
