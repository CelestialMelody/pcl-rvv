...
{
  Eigen::Affine3f transform = Eigen::Affine3f::Identity();
  transform.scale(Eigen::Vector3f(0.55f, 0.60f, 0.65f));
  return transform;
}
...
void
benchPointXYZ(const std::string& name, const pcl::PointCloud<pcl::PointXYZ>& cloud, const Eigen::Affine3f& transform)
{
...
void
benchSubsetFallback(const std::string& name, const pcl::PointCloud<pcl::PointXYZ>& cloud, const Eigen::Affine3f& transform)
{
  pcl::Indices subset;
...
void
benchPointXYZIFallback(const std::string& name, const pcl::PointCloud<pcl::PointXYZI>& cloud, const Eigen::Affine3f& transform)
{
  Benchmarker bench(name);
...
{
  printBanner('=');
  std::cout << " PCL filters/box_clipper3D Benchmark (BoxClipper3D<PointXYZ> full-cloud RVV path)\n";
  printBanner('=');
...
#endif
  std::cout << "Dataset: synthetic PointXYZ clouds; full-cloud affine box clipping RVV cases and subset/type fallback cases\n";
  ...
  const Eigen::Affine3f balanced_box = makeBalancedBox();
  const Eigen::Affine3f mostly_keep_box = makeMostlyKeepBox();

  benchPointXYZ("box_clipper3D pointxyz balanced 64K", cloud64k, balanced_box);
  benchPointXYZ("box_clipper3D pointxyz balanced 1M", cloud1m, balanced_box);
  benchPointXYZ("box_clipper3D pointxyz mostly-keep 1M", cloud1m, mostly_keep_box);
  benchSubsetFallback("box_clipper3D subset fallback 1M", cloud1m, balanced_box);
  benchPointXYZIFallback("box_clipper3D pointxyzi fallback 1M", cloudXYZI, balanced_box);
