...
- 模块依据：`doc-rvv/library-screening/filters/filters-module-followup-rescreen.zh.md` 的后续执行清单第二项。
...
- 非 `pcl::PointXYZ` 点类型：`PointXYZI` 等虽有 x/y/z 字段，但不是本轮评估覆盖的 ABI / 实例化边界；
...
- 公开 `applyFilter` 仍负责构造 6 个平面，之后用 `if constexpr (pcl::kFrustumCullingPointXYZCompatible<PointT>)` 短路调用 RVV；
...
| `PointXYZI` 等类型字段兼容但未评估 | 初版 trait 允许标准 x/y/z float 类型，bench 发现 `PointXYZI` 误入 RVV；最终收窄为 `std::is_same_v<PointT, pcl::PointXYZ>`，并在评估和文档记录 |
...
- `Dataset: synthetic PointXYZ clouds; full-cloud six-plane frustum RVV cases and subset/type fallback cases`
- `Iterations: 30`
...
| `frustum_culling pointxyzi fallback 1M` | `FrustumCulling<PointXYZI>` | 1M `PointXYZI` | 类型 fallback 语义 / 成本 |

## 当前状态

- 函数级评估：完成。
- RVV 实现：已接入 `PointXYZ` dense 全云主路径。
...
设备：`Milkv-Jupiter`。数据集：synthetic `PointXYZ` clouds，full-cloud six-plane frustum RVV cases and subset/type fallback cases。Iterations: `30`。
...
| `frustum_culling pointxyzi fallback 1M` | 76.4250 | 76.3607 | 1.00x | 非 `PointXYZ` 类型 fallback，不作为 RVV 性能结论 |

结论：`FrustumCulling<PointXYZ>::applyFilter(Indices&)` dense 全云主路径在板卡上约 `4.33x` 到 `5.94x`。fallback case 只证明未覆盖路径保持标量语义和接近成本。
