...
runFilter(const pcl::PointCloud<pcl::PointXYZ>::ConstPtr& cloud,
          float sigma_s,
          float sigma_r,
          unsigned int threads)
{
  pcl::FastBilateralFilterOMP<pcl::PointXYZ> filter(threads);
...
TEST(FastBilateralOMPRVV, LargeOrganizedCloudProducesFiniteSmoothedDepth)
{
  const auto cloud = makeOrganizedCloud(96, 72, false);
  const auto output = runFilter(cloud, 5.0f, 0.05f, 2);
...
TEST(FastBilateralOMPRVV, NonFiniteDepthIsReplacedBeforeFiltering)
{
  const auto cloud = makeOrganizedCloud(80, 64, true);
  const auto output = runFilter(cloud, 4.0f, 0.04f, 2);
...
TEST(FastBilateralOMPRVV, SmallCloudFallbackKeepsOrganizedOutput)
{
  const auto cloud = makeOrganizedCloud(7, 5, true);
  const auto output = runFilter(cloud, 3.0f, 0.05f, 2);
...
TEST(FastBilateralOMPRVV, SingleThreadAfterRVVMainPathKeepsFallbackSemantics)
{
  const auto large = makeOrganizedCloud(96, 72, true);
  const auto large_output = runFilter(large, 5.0f, 0.05f, 2);
  expectFiniteZ(large_output);

  const auto small = makeOrganizedCloud(7, 5, true);
  const auto small_output = runFilter(small, 3.0f, 0.05f, 1);

  ASSERT_EQ(small_output.size(), small->size());
  expectFiniteZ(small_output);
}

TEST(FastBilateralOMPRVV, NonOrganizedInputReturnsEmptyOutput)
{
  auto cloud = makeOrganizedCloud(32, 2, false);
  cloud->height = 1;
  cloud->width = static_cast<std::uint32_t>(cloud->size());

  pcl::FastBilateralFilterOMP<pcl::PointXYZ> filter(2);
  filter.setInputCloud(cloud);
  pcl::PointCloud<pcl::PointXYZ> output;
  filter.filter(output);
