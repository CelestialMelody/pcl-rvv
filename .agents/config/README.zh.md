...
skill（技能）或 reference（参考文档）正文作为固定规则。通用 agent instructions（agent 指令体系）正文只引用配置键、
...
4. 在 S0 输出 `preferences_loaded`，并冻结注释、文档、证据、日志、instruction_feedback（指令反馈）和环境偏好。
...
- `defaults.yaml` 里的当前默认值可以保留；其它 agent instruction 正文应优先写 `paths.*`、`artifact_layout.*`、`test_support.*`、`evidence.*` 或 env var 名。
...
- 提交 agent instruction patch 前运行一次硬编码扫描，并把命中分成 `config default keep`、`historical fact keep with boundary`、`rewrite to config key` 三类；无法分类的命中先不要提交。
...
| `agent_asset_patch` | separate-review-required（单独审查） | 作为 agent instruction patch，必须与 topic 产物拆分 |

Handoff Packet（交接数据包）中的 `dirty_isolation`、`artifacts_created_or_updated` 和 `commit_preferences` 应引用这些 class（类别）或说明 prompt override（提示词覆盖），不要把本轮 topic 输出和 agent instruction patch 混成一个提交边界。
