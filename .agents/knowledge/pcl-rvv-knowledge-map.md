...
本文件是给 Codex 和未来 `rvv-agent` 使用的轻量读取索引。它只说明
...
| --- | --- | --- | --- |
| `repo_rules`（仓库规则） | `AGENTS.md`、`.agents/skills/` | 持久仓库约定和可复用 agent workflow。 | 先读适用的 `AGENTS.md`，再读匹配的 skill。 |
| `normative_rvv_knowledge`（通用 RVV 知识） | `doc-rvv/rvv/` | 跨 topic 的 RVV 知识、数学函数向量化说明和可复用 RVV 约定。 | 需要通用 RVV 规则时，优先从这里选择窄文件读取。 |
| `historical_topic_reports`（历史 topic 报告） | `doc-rvv/<module>/` | topic 文档、closeout 记录、实现说明和历史决策。 | 只读当前模块或 topic 相关文档，并用当前源码复核。 |
| `screening_and_queue_docs`（筛选和队列文档） | `doc-rvv/library-screening/` | 模块筛选、候选队列、状态表和下一主题建议。 | 选择、恢复或复筛 topic 时读取。 |
| `executable_evidence`（可执行证据） | `test-rvv/<module>/<topic>/` | test、bench、QEMU、反汇编检查、board 脚本和日志入口。 | 只读取当前结论需要的具体测试、脚本或日志。 |
| `source_truth`（当前源码真相） | 当前 PCL 源码、当前 git diff、`__RVV10__` 路径 | 当前实现行为的权威来源。 | 下结论前必须用当前源码和当前 diff 复核文档与测试。 |
| `migration_only`（仅迁移材料） | `chats/`、`.skills/` | 历史 prompt 材料和旧 skill 草稿。 | 正常 topic runtime 不读取；只有用户要求迁移或追溯时才读取。 |

## Recommended Retrieval Order（推荐读取顺序）

1. 读取 `AGENTS.md`，确认仓库级边界。
2. 选择匹配的 `.agents/skills/<skill-name>/SKILL.md`。
3. 读取本知识索引，决定下一步需要打开哪类证据。
4. 如果任务是选题或恢复工作，读取 `doc-rvv/library-screening/` 中的状态和队列文档。
5. 如果任务需要通用 RVV 规则，读取 `doc-rvv/rvv/` 下的相关窄文件。
6. 如果任务针对已有 topic，读取 `doc-rvv/<module>/` 下的对应 topic 文档。
7. 用当前源码和当前 git diff 复核文档结论。
8. 只在需要证明具体结论时，读取 `test-rvv/<module>/<topic>/` 下的测试、bench、脚本或日志。
...
- 正常 RVV topic 工作中不要读取 `chats/` 或旧 `.skills/`。
...
- `.codex/` 只放 Codex 专用配置。
- `chats/` 和 `.skills/` 保持 migration-only（仅迁移材料），除非用户明确要求清理或追溯。
