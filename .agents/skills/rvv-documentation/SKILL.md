...
- 复杂 topic 的 topic navigation、testing overview、correctness tests、benchmark/evidence、optimization evidence、optimization roadmap、test-support code map、phase suite 和 evaluation 应优先按 `doc-suite-quality-bar.zh.md` 与 `references/templates/template-index.zh.md` 审计。role template（职责模板）不接管文件名；新增或重命名 topic-local 文档时先查 `artifact_layout`，需要跨 topic 稳定命名时先更新配置。成熟 sibling topic（同模块相邻主题）只作为 optional calibration（可选校准样例），不得成为必须点名的规范源，也不得复制其 topic-specific 算法、数值、phase 名或结论。
