...
- topic-local doc suite 的 role-based templates（基于职责的模板）：见 [references/templates/template-index.zh.md](references/templates/template-index.zh.md)。模板只定义职责、内容结构、裁剪规则和 closeout checks；最终路径和命名优先服从 `.agents/config/defaults.yaml` 的 `artifact_layout`。
...
- 评估文档负责决策审计；`artifact_layout.topic_doc_template` 解析出的 production 长期主题文档负责 production 长期维护，只有存在 adopted production behavior（已采用生产行为）、production patch（生产补丁）或 PI5 生产证据闭环通过后才适用。
...
- 多阶段优化 topic 应在 `artifact_layout.optimization_roadmap_template` 解析出的 topic-local 路径或当前配置等价位置维护主题级优化路线图。路线图只记录候选家族、搜索空间、阶段反思、新增想法、优先级和恢复条件；不要把它写成阶段流水，也不要替代 evaluation 的取舍审计。
- 复杂 topic 的 topic navigation、testing overview、correctness tests、benchmark/evidence、optimization evidence、optimization roadmap、test-support code map、phase suite 和 evaluation 应优先按 `doc-suite-quality-bar.zh.md`、`references/templates/template-index.zh.md` 和 `.agents/config/defaults.yaml` 的 `artifact_layout` role path key 审计。role template（职责模板）不接管文件名；默认路径来自配置，当前 topic 已有稳定路径可保留并写入 `doc_suite_role_inventory`。成熟 sibling topic（同模块相邻主题）只补充校准读者路径和结构完整度，不得成为必须点名的规范源，也不得复制其 topic-specific 算法、数值、phase 名、文件名或结论。
...
- `diagnostic`、`bench-only`、`rollback/no-production` 或未接 production 的 `partial-production-candidate` 不默认创建 `artifact_layout.topic_doc_template` 解析出的 production 长期主题文档。这些结论的“诊断证据链”主归属是 topic-local evaluation、phase result、roadmap / matrix 和 Handoff；若已有 production 长期主题文档仅承载诊断或 no-production 结论，应删除或标为 not_applicable，除非用户明确要求保留历史归档。
...
production 长期主题文档修改后，如果影响生产接入结论、bench case 含义、新增诊断失败、workflow 规则或筛选状态，应同步函数级评估、模块工作日志和相关筛选状态表。no-production closeout 修改只同步 topic-local evaluation / phase 文档和状态表，不因此新建 `artifact_layout.topic_doc_template` 解析出的文档。

进入 production integration loop、准备最终 closeout 或准备提交前，若当前 topic 已有 `artifact_layout.topic_doc_template` 解析出的 production 长期主题文档，worker 必须先用当前 production diff、phase result 和 Evidence Doctor 做 freshness check；如果长期文档与当前 truth 不一致，先标记 `stale_doc_pending_refresh` 或刷新后再继续后续闭环。普通 diagnostic phase 不要求每轮都做完整 production 长期文档比对。
