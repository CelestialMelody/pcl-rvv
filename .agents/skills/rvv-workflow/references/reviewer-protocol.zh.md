...
- 如果 staged set 或工作区 diff 包含 topic-local / module-local `.gitignore`，reviewer 必须检查它只忽略该 topic / module 的额外生成物；若它反忽略配置解析出的 evidence output subdir 中的 summary、doctor、manifest、registry 或 raw log，应要求改回 explicit `git add -f <specific files>` 的 evidence commit 模型。
