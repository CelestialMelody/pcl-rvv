...
- 若使用 `no-tree-vectorize` 或同类局部优化限制，评估、适用的 production 长期主题文档和源码注释都要说明限制范围、保护的语义和反汇编证据。
...
- closeout 同步评估文档、topic-local phase / diagnostic 文档、适用的 production 长期主题文档、模块工作日志和状态表；no-production 不默认创建 `doc-rvv`。
