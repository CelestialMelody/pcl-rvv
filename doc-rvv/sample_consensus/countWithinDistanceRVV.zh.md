...
- **不带法线(Plane)** 内点条件：
  $$
  |ax + by + cz + d| \le threshold
  $$
- **带法线 (NormalPlane)** 内点条件：
  $$
  |w \cdot d_{ang} + (1 - w) \cdot d_{geom}| \le threshold
  $$
...
$$
d = |{dist\_to\_centor} - r| \le threshold
$$
...
3. **判断逻辑**：若 $r_{min}^2 \le$  $d_{sq}$  $ \le r_{max}^2$，则该点为内点。
...
| `__riscv_vmfge_vf_f32m2_b16(v, float scalar, vl)`                           | **向量-标量比较（大于等于）**：`mask[i] = (v[i] >= scalar)`                                                                                           | `v`: float 向量；`scalar`: 标量阈值；`vl`: 向量长度                          | `vbool16_t`        | 用于 `sqr_dist >= sqr_inner_radius` |
| `__riscv_vmfle_vf_f32m2_b16(v, float scalar, vl)`                           | **向量-标量比较（小于等于）**：`mask[i] = (v[i] <= scalar)`                                                                                           | `v`: float 向量；`scalar`: 标量阈值；`vl`: 向量长度                          | `vbool16_t`        | 用于 `sqr_dist <= sqr_outer_radius` |
