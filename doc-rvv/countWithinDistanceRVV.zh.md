...
这与 `PCLAT` 的逻辑完全一致，基于 PCL 的 **数据对齐假设**：

- `input_` (点) 和 `normals_` (法线) 是两个独立的数组，但它们的大小相同，且索引一一对应。
- `Point[100]` 的法线必然存储在 `Normal[100]` 中。

**推论（索引复用）**：同一个 Real ID = (indices)[i]，既用于在 input 中找坐标，也用于在 normals_ 中找法线。这意味着在 RVV 中，我们只需要加载一次索引向量。
