...
| ---: | --- | --- | --- | --- | --- |
| 1 | approximate PMF grid/open | `impl/approximate_progressive_morphological_filter.hpp` | `ApproximateProgressiveMorphologicalFilter::extract` 中 grid z-min 与 height threshold tail | 已采纳；当前暂停 | 已建立 test-rvv topic 并完成 production integration loop。接入后的 production public board 5-run 显示 `PointXYZ` dense median 1.60x、non-dense median 1.35x；040 点型扩展显示 `PointXYZI` 1.52x / 1.42x、`PointXYZRGB` 1.50x / 1.43x、`PointXYZRGBA` 1.51x / 1.43x，Evidence Doctor 均为 Errors=0 / Warnings=0 / Suggestions=0。当前 production patch 为 adopted production behavior。window-open RVV 不接入 production。050 tail 剩余比较 / 输出压缩探针相对 040 RVV 基线整体 neutral，已回退；当前无值得自动推进的下一生产优化方向。 |
| 2 | polygonal prism predicate | `impl/extract_polygonal_prism_data.hpp` | `ExtractPolygonalPrismData::segment` 的 height mask + point-to-plane + polygon predicate | 已采纳；继续后续优化判断 | 已建立 `test-rvv/segmentation/extract_polygonal_prism_data` topic，并完成 clean split production patch。production public board 5-run 显示 dense median 1.75x、indexed median 1.75x，Evidence Doctor 均为 Errors=0 / Warnings=0 / Suggestions=0。当前 production patch 为 adopted production behavior，正式文档为 `doc-rvv/segmentation/extract_polygonal_prism_data-RVV.zh.md`。后续可按 phase 继续评估 concave hull 多 polygon XOR、更多点型和规模阈值。 |

### 7.2 保留实施的候选文件

| 顺序 | 主题 | 主文件 | 推荐入口 / 第一 RVV 目标 | 状态 | 当前结论 / 下一步条件 |
| ---: | --- | --- | --- | --- | --- |
| 3 | GrabCut staging / n-link | `impl/grabcut_segmentation.hpp` | organized beta / n-link weight 或 color staging component | 保留 | 需先补 test 和 component timing，证明 graph solve 之外片段占比。 |
| 4 | GrabCut GMM / max-flow boundary | `src/grabcut_segmentation.cpp` | GMM build/learn probability loops，不动 max-flow 状态机 | 保留 | 与模板头同主题评估；max-flow 仅记录为主成本稀释源。 |
...
| 6 | organized multi-plane | `impl/organized_multi_plane_segmentation.hpp` | `plane_d` dot loop、boundary gather、projection loop | 保留 | `refine` label growth 和 region plane fitting 暂缓；先做 component ablation。 |
...
| 8 | min-cut potentials | `impl/min_cut_segmentation.hpp` | unary / binary potential component | 保留 | 有上游 test，适合 component ablation；solver 稀释比例是首问题。 |
...
| RVV 实现 | 第一主题已有 adopted production behavior；040 点型扩展已采纳；050 进一步 tail 压缩探针已拒绝并回退。第二主题 `impl/extract_polygonal_prism_data.hpp` 已完成 S11 production closeout，当前 production patch 为 adopted production behavior。 |
| 专项测试 | 已创建 `test-rvv/segmentation/approximate_progressive_morphological_filter` 和 `test-rvv/segmentation/extract_polygonal_prism_data`。第二主题 Std 2 tests、RVV 11 tests 通过，板卡 correctness 11 tests 通过。 |
| bench | approximate PMF 已运行 production public 板卡 5-run。polygonal prism 已运行 diagnostic dense / indexed 和 production public dense / indexed 板卡 5-run，production public 两组 median 均为 1.75x。 |
| QEMU / 反汇编 / 目标硬件闭环 | 第一主题已完成 QEMU correctness/log-shape、production asm、040 点型扩展板卡 Evidence Doctor 和 050 RVV-vs-RVV board A/B。第二主题已完成 QEMU smoke、production `segmentRvv` 反汇编归属、production public 板卡 repeated 和 Evidence Doctor 0 / 0 / 0。 |
| 第一条未完成主题 | `impl/extract_polygonal_prism_data.hpp` 已采纳当前 single polygon dense / indexed production RVV 路径。若继续当前 topic，下一步默认开 concave hull XOR 或点类型扩展 phase；若暂停本 topic，则队列下一主题为 GrabCut staging / n-link。 |
...
- 第一条建议主题 `impl/approximate_progressive_morphological_filter.hpp` 已完成当前授权范围内的生产优化闭环：040 已采纳，050 已尝试并回退。第二条建议主题 `impl/extract_polygonal_prism_data.hpp` 已完成生产接入闭环 PI2-PI5 和 S11 closeout，当前 single polygon dense / indexed production RVV 路径已采纳。
