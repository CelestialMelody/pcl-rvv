...
| bench 诊断主题                    |   19 | 有局部 RVV 实验问题，按诊断成熟度分为可直接建 bench、待诊断设计、保留 / 按需诊断或已完成 |
...
| `fast_bilateral`        | z 预处理整体约 `1.04x`~`1.10x`                                                                                      | lattice blur bench-diagnosis RVV 为 `1.00x`，生产 blur 回退                                      | 只覆盖小前置片段通常是弱收益；邻域 / lattice / 冲突累加不因循环大自动升级         |
...
- bench 诊断按成熟度分层，不再用“6.2 bench 诊断主题 / 6.3 观察暂不建 bench”硬切分；有局部 RVV 点但诊断问题尚不成熟的主题仍属于 bench 诊断候选，只是状态为 `待诊断设计`；
...
| bench 诊断主题     |     19 | 有局部 RVV 点或可隔离诊断问题；用成熟度字段区分可直接建 bench 与待诊断设计   |
| 暂缓主题           |      1 | filters 文件本身不承载主要热点或当前不适合作为 filters 专项                  |
...
| `shadowpoints`        | `ShadowPoints::applyFilter(Indices&)`、`applyFilter(PointCloud&)`                 | `direct-main-path`                 | point + normal 双 AoS dot、`abs` threshold、`negative_`、保序压缩                      | 双输入 stride load、cloud-out 整点复制、keep_organized 坏点写                 | 直接线性几何判定，可建专项和 bench；实际完成后已转为 bench-diagnosis 生产回退，状态见第 6 节 |
...
本节合并原 `bench 诊断主题` 与 `观察 / 暂不建 bench`。两者本质上都是“有局部 RVV 点或可隔离诊断问题，但当前不直接承诺生产分流”；区别用 `诊断成熟度` 表达：
...
- `保留 / 按需诊断`：诊断方向明确但不再默认逐项推进，需等待使用场景、profile、数据集或新方案；
- `保留 / 待诊断设计`：有局部 RVV 点，但需要先证明 bench 问题能代表真实入口。
...
| `extract_indices`                 | `ExtractIndices::applyFilterIndices`、`applyFilter`、`filterDirectly` | `diagnostic-only`    | 已完成（生产不接入）              | bitmap / set-difference 替代方案与 keep_organized 坏点写    | bitmap scan 局部收益明显，但 keep_organized full diagnostic / production 不成立；生产入口不接入本主题分流                                                                                   |
...
| `model_outlier_removal`           | `ModelOutlierRemoval::applyFilterIndices`                                 | `tail-compress`      | 保留 / 按需诊断                  | `getDistancesToModel` 后的 threshold + compress             | `getDistancesToModel` 和模型多态主导，后处理占比未知；仅在距离生成成本已可隔离或模型侧已有热点证据时推进                                                                                    |
| `normal_space`                    | `NormalSpaceSampling::applyFilter`、`findBin`                           | `partial-preprocess` | 保留 / 按需诊断                  | normal bin id 预计算                                       | list/bin/random sampling 主导，生产接入需重构采样状态；仅在固定 bin 预处理占比明确时推进                                                                                                     |
| `radius_outlier_removal`          | `RadiusOutlierRemoval::applyFilterIndices`                                | `tail-compress`      | 保留 / 按需诊断                  | `to_keep` 到 indices / removed_indices 的尾段压缩          | nearestK/radius search 主导，尾段压缩只能给出成本上界；仅在 search 后处理被证明是瓶颈时推进                                                                                                   |
| `sampling_surface_normal`         | `findXYZMaxMin`、`computeMeanAndCovarianceMatrix`、`partition`        | `partial-preprocess` | 保留 / 按需诊断                  | min/max 与小分区 covariance 规约                            | recursive partition、random、Eigen plane solve 主导；仅在分区 covariance / minmax 成为独立热点时推进                                                                                          |
| `statistical_outlier_removal`     | `StatisticalOutlierRemoval::applyFilterIndices`                           | `tail-compress`      | 保留 / 按需诊断                  | distances mean/stddev 规约与 threshold compress             | KNN search 主导，整体收益预计有限；仅在距离数组已存在且统计尾段占比明确时推进                                                                                                                |
| `uniform_sampling`                | `UniformSampling::applyFilter(Indices&)`                                  | `partial-preprocess` | 保留 / 按需诊断                  | leaf id 与 voxel center distance                            | `leaves_` map 和 per-leaf conflict update 主导；仅在 leaf-id / distance 片段能覆盖真实入口主成本时推进                                                                                       |
| `src/voxel_grid_label.cpp`        | `VoxelGridLabel::applyFilter(PointCloud&)`                                | `partial-preprocess` | 保留 / 按需诊断                  | distance filter 与 leaf id 计算                             | sort、label histogram、`std::map` 和字段聚合主导，固定点类型适合后续按具体数据集做局部诊断                                                                                                   |
| `convolution_3d`                  | `Convolution3D::convolve`、`GaussianKernel::operator()`                 | `diagnostic-only`    | 已完成（生产不接入）            | 邻域 distances threshold、权重、加权求和                    | kernel-only 板卡约 `1.85x`，但 full diagnostic 仅约 `1.09x`~`1.11x`，未改生产入口约 `1.00x`；`radiusSearch` 与不规则邻域访问稀释收益，保留 bench 诊断证据 |
| `covariance_sampling`             | `initCompute`、`computeCovarianceMatrix`、`applyFilter(Indices&)`     | `partial-preprocess` | 保留 / 待诊断设计              | centroid、scaled point、6D vector 构造                      | Eigen 6x6、list sort、采样状态主导；需先隔离 solver、list 状态和采样随机性成本                                                                                                               |
| `crop_hull`                       | `CropHull::applyFilter2D`、`applyFilter3D`                              | `diagnostic-only`    | 保留 / 待诊断设计              | 固定 hull 的 polygon / ray triangle 判定                    | 多 polygon、多 ray、crossing 语义复杂，fixed hull microbench 泛化性弱；需限定常见 hull 形态并证明判定循环占主成本                                                                            |
| `farthest_point_sampling`         | `FarthestPointSampling::applyFilter(Indices&)`                            | `diagnostic-only`    | 保留 / 待诊断设计              | 每轮距离数组更新和 max 查找                                 | 每轮依赖上一采样点且 OpenMP reduction / 随机起点影响语义；需证明能保持采样顺序且距离更新 + max 是主要瓶颈                                                                                   |
| `local_maximum`                   | `LocalMaximum::applyFilterIndices`                                        | `diagnostic-only`    | 保留 / 待诊断设计              | 投影后局部 z 比较尾段                                      | projection、radius search、visited 标记主导；需获得 search 后连续邻域数据形态并证明尾段比较占比                                                                                             |
| `median_filter`                   | `MedianFilter::applyFilter(PointCloud&)`                                  | `diagnostic-only`    | 保留 / 待诊断设计              | organized window z 收集、finite mask、移动限幅              | `nth_element` median 排序类操作主导；需提出不改变 median 语义且代表真实窗口成本的诊断设计                                                                                                    |
| `morphological_filter`            | `applyMorphologicalOperator`                                              | `diagnostic-only`    | 保留 / 待诊断设计              | boxSearch 后 search-result min/max                         | octree boxSearch 主导，且已有 `getMinMax3D` 间接受益；需确认 search-result 数组形态和 min/max 占比                                                                                          |
| `voxel_grid_occlusion_estimation` | `VoxelGridOcclusionEstimation` ray traversal / occlusion APIs             | `diagnostic-only`    | 保留 / 待诊断设计              | 坐标转换、ray step、box intersection                        | ray traversal 状态机和 occupancy 访问不规则；需把 ray math 与 occupancy 访问合成可归因诊断 case                                                                                              |
...
|    3 | `shadowpoints`        | `filters/include/pcl/filters/impl/shadowpoints.hpp`                         | `applyFilter(Indices&)` 的 point + normal dot、`abs` threshold、`negative_` | 已完成（bench-diagnosis，生产回退） | `test-rvv/filters/shadowpoints/shadowpoints-evaluation.zh.md`               | `doc-rvv/filters/shadowpoints-RVV.zh.md`        | bench-diagnosis RVV helper 正确且指令命中，但 Milkv-Jupiter 主诊断 case 仅 `0.44x`~`0.63x`，不接入生产；上游源码已回退，实验保留在 `test-rvv`；fallback case 约 `1.00x`                                                   |
...
本节合并原 `6.2 bench 诊断主题` 与 `6.3 观察 / 暂不建 bench`。两类主题的共同点是局部 RVV 点或诊断问题不能直接等价于生产主路径收益；区别改由状态字段表达成熟度。`保留 / 按需诊断` 不是默认继续队列，只有出现明确使用场景、profile、数据集或新方案时再取出；`保留 / 待诊断设计` 需要先证明 bench 问题能代表真实入口，再决定是否建立专项目录。bench 诊断主题默认不修改公开 API、不接入生产分流；只有 full diagnostic 或生产入口在板卡上稳定明显收益，且覆盖面、fallback 和维护成本同时成立，才重新纳入生产候选。
...
|    5 | `model_outlier_removal`           | 连续 distances 后的 threshold + compress microbench                                                             | 保留 / 按需诊断                      | 待建                                                                                                                                                                                                                          | `getDistancesToModel` 和模型多态主导，后处理占比未知；仅在距离生成成本已可隔离或模型侧已有热点证据时推进                                                                                                                                         |
|    6 | `normal_space`                    | normal bin id 预计算 microbench                                                                                 | 保留 / 按需诊断                      | 待建                                                                                                                                                                                                                          | list/bin/random sampling 主导，生产接入需重构采样状态；仅在固定 bin 预处理占比明确时推进                                                                                                                                                          |
|    7 | `radius_outlier_removal`          | `to_keep` 到 indices / removed_indices 的尾段压缩 microbench                                                  | 保留 / 按需诊断                      | 待建                                                                                                                                                                                                                          | nearestK/radius search 主导，尾段压缩只能给出成本上界；仅在 search 后处理被证明是瓶颈时推进                                                                                                                                                        |
|    8 | `sampling_surface_normal`         | min/max 与小分区 covariance 规约诊断                                                                            | 保留 / 按需诊断                      | 待建                                                                                                                                                                                                                          | recursive partition、random、Eigen plane solve 主导；仅在分区 covariance / minmax 成为独立热点时推进                                                                                                                                               |
|    9 | `statistical_outlier_removal`     | distances mean/stddev 规约与 threshold compress                                                                 | 保留 / 按需诊断                      | 待建                                                                                                                                                                                                                          | KNN search 主导，整体收益预计有限；仅在距离数组已存在且统计尾段占比明确时推进                                                                                                                                                                     |
|   10 | `uniform_sampling`                | leaf id 与 voxel center distance microbench                                                                     | 保留 / 按需诊断                      | 待建                                                                                                                                                                                                                          | `leaves_` map 和 per-leaf conflict update 主导；仅在 leaf-id / distance 片段能覆盖真实入口主成本时推进                                                                                                                                            |
|   11 | `src/voxel_grid_label.cpp`        | distance filter 与 leaf id 计算 microbench                                                                      | 保留 / 按需诊断                      | 待建                                                                                                                                                                                                                          | sort、label histogram、`std::map` 和字段聚合主导，固定点类型适合后续按具体数据集做局部诊断                                                                                                                                                        |
...
|   13 | `covariance_sampling`             | centroid、scaled point、6D vector 构造                                                                          | 保留 / 待诊断设计                    | 待建                                                                                                                                                                                                                          | Eigen 6x6、list sort、采样状态主导；需先隔离 solver、list 状态和采样随机性成本                                                                                                                                                                     |
|   14 | `crop_hull`                       | 固定 hull 的 polygon / ray triangle 判定                                                                        | 保留 / 待诊断设计                    | 待建                                                                                                                                                                                                                          | 多 polygon、多 ray、crossing 语义复杂，fixed hull microbench 泛化性弱；需限定常见 hull 形态并证明判定循环占主成本                                                                                                                                |
|   15 | `farthest_point_sampling`         | 距离数组更新和 max 查找                                                                                         | 保留 / 待诊断设计                    | 待建                                                                                                                                                                                                                          | 每轮依赖上一采样点且 OpenMP reduction / 随机起点影响语义，生产接入侵入性高；需保持采样顺序语义并证明距离更新 + max 是主要瓶颈                                                                                                                    |
|   16 | `local_maximum`                   | 固定邻域 z 比较                                                                                                 | 保留 / 待诊断设计                    | 待建                                                                                                                                                                                                                          | projection、radius search、visited 标记主导，局部比较覆盖真实入口太小；需获得 search 后连续邻域数据形态并证明尾段比较占比                                                                                                                       |
|   17 | `median_filter`                   | organized window z 收集和 finite mask                                                                           | 保留 / 待诊断设计                    | 待建                                                                                                                                                                                                                          | `nth_element` median 排序类操作主导，window gather 诊断暂不能说明整体收益；需提出不改变 median 语义且代表真实窗口成本的诊断设计                                                                                                                  |
|   18 | `morphological_filter`            | search-result min/max                                                                                           | 保留 / 待诊断设计                    | 待建                                                                                                                                                                                                                          | octree boxSearch 主导，且已有 `getMinMax3D` 间接受益；需确认 search-result 数组形态和 min/max 占比                                                                                                                                                |
|   19 | `voxel_grid_occlusion_estimation` | ray step、坐标转换、box intersection                                                                            | 保留 / 待诊断设计                    | 待建                                                                                                                                                                                                                          | ray traversal 状态机和 occupancy 访问不规则，ray math microbench 与真实输出状态解耦过强；需把 ray math 与 occupancy 访问合成可归因诊断 case                                                                                                     |
...
本阶段 filters follow-up 已完成直接主路径候选闭环：6.1 的直接主路径候选全部完成。统一后的 bench 诊断主题也已有四类代表结果：
...
因此 filters 模块后续不再用旧 `6.2 bench 诊断主题` / `6.3 观察 / 暂不建 bench` 做硬区分。所有有局部 RVV 点或可隔离实验问题的主题统一放入 bench 诊断主题，再用状态表达成熟度。后续策略改为：

- 已完成主题进入维护和复核阶段，优先处理文档、日志、提交拆分和已发现问题；
- `保留 / 按需诊断` 主题不是默认继续队列，只有在上游使用场景、profile、数据集或新方案能证明“目标片段接近入口主成本”时再启动；
- `保留 / 待诊断设计` 主题需要先提出可隔离、可归因、可板卡验证的诊断问题，再决定是否建立 `test-rvv` 专项目录；
