...
| `radius_outlier_removal`          | `RadiusOutlierRemoval::applyFilterIndices`                                | `tail-compress`        | 保留 / 待诊断                    | `to_keep` 到 indices / removed_indices 的尾段压缩           | nearestK/radius search 主导，尾段压缩只能给出成本上界；仅在 search 后处理被证明是瓶颈时推进                                                                         |
...
| `covariance_sampling`             | `initCompute`、`computeCovarianceMatrix`、`applyFilter(Indices&)`     | `partial-preprocess`   | 保留 / 待诊断                    | centroid、scaled point、6D vector 构造                        | Eigen 6x6、list sort、采样状态主导；需先隔离 solver、list 状态和采样随机性成本                                                                                      |
| `crop_hull`                       | `CropHull::applyFilter2D`、`applyFilter3D`                              | `diagnostic-only`      | 保留 / 待诊断                    | 固定 hull 的 polygon / ray triangle 判定                      | 多 polygon、多 ray、crossing 语义复杂，fixed hull microbench 泛化性弱；需限定常见 hull 形态并证明判定循环占主成本                                                   |
| `farthest_point_sampling`         | `FarthestPointSampling::applyFilter(Indices&)`                            | `diagnostic-only`      | 保留 / 待诊断                    | 每轮距离数组更新和 max 查找                                   | 每轮依赖上一采样点且 OpenMP reduction / 随机起点影响语义；需证明能保持采样顺序且距离更新 + max 是主要瓶颈                                                           |
| `local_maximum`                   | `LocalMaximum::applyFilterIndices`                                        | `diagnostic-only`      | 保留 / 待诊断                    | 投影后局部 z 比较尾段                                         | projection、radius search、visited 标记主导；需获得 search 后连续邻域数据形态并证明尾段比较占比                                                                     |
| `median_filter`                   | `MedianFilter::applyFilter(PointCloud&)`                                  | `diagnostic-only`      | 保留 / 待诊断                    | organized window z 收集、finite mask、移动限幅                | `nth_element` median 排序类操作主导；需提出不改变 median 语义且代表真实窗口成本的诊断设计                                                                         |
| `morphological_filter`            | `applyMorphologicalOperator`                                              | `diagnostic-only`      | 保留 / 待诊断                    | boxSearch 后 search-result min/max                            | octree boxSearch 主导，且已有 `getMinMax3D` 间接受益；需确认 search-result 数组形态和 min/max 占比                                                                |
| `voxel_grid_occlusion_estimation` | `VoxelGridOcclusionEstimation` ray traversal / occlusion APIs             | `diagnostic-only`      | 保留 / 待诊断                    | 坐标转换、ray step、box intersection                          | ray traversal 状态机和 occupancy 访问不规则；需把 ray math 与 occupancy 访问合成可归因诊断 case                                                                     |
...
|    7 | `radius_outlier_removal`          | `to_keep` 到 indices / removed_indices 的尾段压缩 microbench                                                                    | 保留 / 待诊断                    | 待建                                                                                                                                                                                                                          | nearestK/radius search 主导，尾段压缩只能给出成本上界；仅在 search 后处理被证明是瓶颈时推进                                                                                                                                                       |
...
|   13 | `covariance_sampling`             | centroid、scaled point、6D vector 构造                                                                                            | 保留 / 待诊断                    | 待建                                                                                                                                                                                                                          | Eigen 6x6、list sort、采样状态主导；需先隔离 solver、list 状态和采样随机性成本                                                                                                                                                                    |
