...
| bench 诊断主题                    |   10 | 有可隔离局部 RVV 实验问题，但当前不直接承诺生产分流                       |
...
- bench 诊断只收纳能回答明确局部问题的候选，不把所有“有局部 loop”的暂缓项自动纳入。
...
| bench 诊断主题      |     10 | 有可隔离局部 RVV 实验问题，但不直接承诺生产分流                 |
...
| ----------------------------------- | ----------------------------------------------------------------------- | ---------------------- | ---------------------------------------------- | ----------------------------------------------------------------------------------------------------------- |
| `bilateral`                       | `BilateralFilter<PointT>::applyFilter`、`computePointWeight`        | `PointXYZI-production` | 邻域距离和 intensity 权重循环                  | 已由 6.3 诊断升级：复用 common `pcl::expf_RVV_f32m2` 后，生产 `PointXYZI` 入口板卡 `2.06x`~`3.43x`，非覆盖类型和小邻域回退标量 |
...
观察主题不是删除项。后续只有当能提出可隔离、可归因、可板卡验证的诊断问题时，才升级为 bench 诊断。
...
6.1 全部完成后，本阶段已优先诊断前三个代表性主题：`approximate_voxel_grid`、`grid_minimum`、`extract_indices`。三者分别覆盖“前置 staging 可升级生产”、“局部片段收益被 sort/min-z 稀释”和“局部 bitmap scan 明显加速但 full / production 不成立”三类结果，已足够支撑 filters 后续策略收敛。剩余主题保留为按需诊断池，不再作为默认顺序继续逐项推进；只有出现明确使用场景、热点证据或新的实现方案时，才从本表取出单独评估。bench 诊断主题默认不修改公开 API、不接入生产分流；只有诊断结果显示板卡收益、覆盖面和维护成本同时成立，才重新纳入生产候选。
...
|    2 | `grid_minimum`                | 2D grid id + floor 预计算 microbench                                                                            | 已完成（bench-diagnosis，生产不接入） | `test-rvv/filters/grid_minimum/grid_minimum-evaluation.zh.md`；`doc-rvv/filters/grid_minimum-RVV.zh.md`；`test-rvv/filters/grid_minimum/output/board/analyze_bench_compare.log`                                         | cell-id 片段板卡约 `1.33x`~`1.52x`，但 full diagnostic 仅约 `1.09x`~`1.14x`，sort 和 per-cell min z 稀释收益；生产入口不接入本主题分流                                                                                                   |
|    3 | `extract_indices`             | bitmap / set-difference 替代方案与 keep_organized 坏点写诊断                                                    | 已完成（bench-diagnosis，生产不接入） | `test-rvv/filters/extract_indices/extract_indices-evaluation.zh.md`；`doc-rvv/filters/extract_indices-RVV.zh.md`；`test-rvv/filters/extract_indices/output/board/analyze_bench_compare.log`                             | bitmap scan 局部片段板卡约 `2.21x`~`4.84x`，但 keep_organized full diagnostic 仅约 `1.22x`~`1.24x` 或 `0.99x`，未修改生产入口约 `0.99x`；生产入口不接入本主题分流                                                                    |
|    4 | `model_outlier_removal`       | 连续 distances 后的 threshold + compress microbench                                                             | 保留 / 按需诊断                      | 待建                                                                                                                                                                                                                          | `getDistancesToModel` 和模型多态主导，后处理占比未知；仅在距离生成成本已可隔离或模型侧已有热点证据时推进                                                                                                                                        |
|    5 | `normal_space`                | normal bin id 预计算 microbench                                                                                 | 保留 / 按需诊断                      | 待建                                                                                                                                                                                                                          | list/bin/random sampling 主导，生产接入需重构采样状态；仅在固定 bin 预处理占比明确时推进                                                                                                                                                         |
|    6 | `radius_outlier_removal`      | `to_keep` 到 indices / removed_indices 的尾段压缩 microbench                                                  | 保留 / 按需诊断                      | 待建                                                                                                                                                                                                                          | nearestK/radius search 主导，尾段压缩只能给出成本上界；仅在 search 后处理被证明是瓶颈时推进                                                                                                                                                       |
|    7 | `sampling_surface_normal`     | min/max 与小分区 covariance 规约诊断                                                                            | 保留 / 按需诊断                      | 待建                                                                                                                                                                                                                          | recursive partition、random、Eigen plane solve 主导；仅在分区 covariance / minmax 成为独立热点时推进                                                                                                                                              |
|    8 | `statistical_outlier_removal` | distances mean/stddev 规约与 threshold compress                                                                 | 保留 / 按需诊断                      | 待建                                                                                                                                                                                                                          | KNN search 主导，整体收益预计有限；仅在距离数组已存在且统计尾段占比明确时推进                                                                                                                                                                    |
|    9 | `uniform_sampling`            | leaf id 与 voxel center distance microbench                                                                     | 保留 / 按需诊断                      | 待建                                                                                                                                                                                                                          | `leaves_` map 和 per-leaf conflict update 主导；仅在 leaf-id / distance 片段能覆盖真实入口主成本时推进                                                                                                                                           |
|   10 | `src/voxel_grid_label.cpp`    | distance filter 与 leaf id 计算 microbench                                                                      | 保留 / 按需诊断                      | 待建                                                                                                                                                                                                                          | sort、label histogram、`std::map` 和字段聚合主导，固定点类型适合后续按具体数据集做局部诊断                                                                                                                                                       |

bench 诊断主题不改变公开 API 和生产分流。只有诊断结果显示板卡收益、覆盖面和维护成本同时成立，才重新纳入生产候选。

### 6.3 观察 / 暂不建 bench

6.3 不是默认实现队列，而是“诊断设计候选池”。状态列表示是否已经形成能代表真实入口的诊断设计；在状态仍为 `待诊断设计` 时，应先评估 full diagnostic 是否成立，再决定是否建专项 bench。

| 主题                                | 状态                 | 局部 RVV 点                                 | 暂不建 bench 的原因                                                                      | 重新考虑条件                                                   |
| ----------------------------------- | -------------------- | ------------------------------------------- | ---------------------------------------------------------------------------------------- | -------------------------------------------------------------- |
| `bilateral`                       | 已完成（`PointXYZI` 生产接入） | 固定邻域距离和 intensity 权重循环           | 已完成 full diagnostic：保留标量 double/libm `exp` 时局部 staging 板卡约 `1.16x`、full 约 `1.04x`；复用 common `pcl::expf_RVV_f32m2` 后 full exp `1.65x`~`1.73x`，生产 `BilateralFilter<PointXYZI>` 入口 `2.06x`~`3.43x`；常规 max abs `3.8e-6`，高对比 max abs `1.53e-5`，低于专项误差预算 | 证据见 `test-rvv/filters/bilateral/bilateral-evaluation.zh.md`、`doc-rvv/filters/bilateral-RVV.zh.md` 和 `test-rvv/filters/bilateral/output/board/analyze_bench_compare.log` |
| `convolution_3d`                  | 保留 / 待诊断设计    | 邻域 distances threshold 与 kernel 加权求和 | `radiusSearch` 与泛型 kernel 主导，kernel-only microbench 暂不能代表 `convolve` 入口 | 能隔离并证明 kernel 计算在真实入口中占主要成本                 |
| `covariance_sampling`             | 保留 / 待诊断设计    | centroid、scaled point、6D vector 构造      | Eigen 6x6、list sort、采样状态主导                                                       | 能先隔离 solver、list 状态和采样随机性成本                     |
| `crop_hull`                       | 保留 / 待诊断设计    | 固定 hull 的 polygon / ray triangle 判定    | 多 polygon、多 ray、crossing 语义复杂，fixed hull microbench 泛化性弱                    | 能限定常见 hull 形态并证明判定循环占主成本                     |
| `farthest_point_sampling`         | 保留 / 待诊断设计    | 距离数组更新和 max 查找                     | 每轮依赖上一采样点且 OpenMP reduction / 随机起点影响语义，生产接入侵入性高               | 能保持采样顺序语义并证明距离更新 + max 是主要瓶颈              |
| `local_maximum`                   | 保留 / 待诊断设计    | 固定邻域 z 比较                             | projection、radius search、visited 标记主导，局部比较覆盖真实入口太小                    | 能获得 search 后连续邻域数据形态并证明尾段比较占比             |
| `median_filter`                   | 保留 / 待诊断设计    | organized window z 收集和 finite mask       | `nth_element` median 排序类操作主导，window gather 诊断暂不能说明整体收益              | 能提出不改变 median 语义且代表真实窗口成本的诊断设计           |
| `morphological_filter`            | 保留 / 待诊断设计    | search-result min/max                       | octree boxSearch 主导，且已有 `getMinMax3D` 间接受益                                   | 能确认 search-result 数组形态和 min/max 占比                   |
| `voxel_grid_occlusion_estimation` | 保留 / 待诊断设计    | ray step、坐标转换、box intersection        | ray traversal 状态机和 occupancy 访问不规则，ray math microbench 与真实输出状态解耦过强  | 能把 ray math 与 occupancy 访问合成可归因诊断 case             |
...
本阶段 filters follow-up 已完成两类闭环：6.1 的直接主路径候选全部完成；6.2 的前三个 bench 诊断主题覆盖了当前最关键的三种不确定性。`approximate_voxel_grid` 证明前置 staging 若能带动 full / production 入口，可以升级为生产路径；`grid_minimum` 证明局部 cell-id 加速会被 sort 与 per-cell min-z 稀释；`extract_indices` 证明局部 bitmap scan 加速不能直接代表 keep_organized full diagnostic 或真实生产入口。

因此 filters 模块当前不再默认继续推进 `model_outlier_removal` 等 6.2 剩余项。后续策略改为：

- 已完成主题进入维护和复核阶段，优先处理文档、日志、提交拆分和已发现问题；
- 6.2 剩余项保留为按需诊断池，只有在上游使用场景、profile、数据集或新方案能证明“目标片段接近入口主成本”时再启动；
- 6.3 观察项同样不删除，但需要先提出可隔离、可归因、可板卡验证的诊断问题；
