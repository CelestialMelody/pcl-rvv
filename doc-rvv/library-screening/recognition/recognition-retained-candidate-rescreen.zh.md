...
| 3 | Geometric consistency ablation | `recognition/include/pcl/recognition/impl/cg/geometric_consistency.hpp` | `clusterCorrespondences()` pairwise consistency predicate | projection/mask 类规则几何经验；源码 O(n^2) 子核 | 已采纳 / topic closeout | 已接入 `clusterCorrespondences()` 的 pairwise predicate production RVV；phase 020 board median `2.510x`，`0/5` 退化，checksum stable。正式文档为 `doc-rvv/recognition/geometric_consistency-RVV.zh.md`；本地统计记录为 `tmp/rvv-topic-stats/3/recognition-geometric_consistency-stats.zh.md`。 |
...
- 当前执行清单已全部收口；若继续 recognition 模块 RVV，应按新的 production-direct phase 重新开题。
