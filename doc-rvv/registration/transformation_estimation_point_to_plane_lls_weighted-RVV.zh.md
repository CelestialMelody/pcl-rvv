...
- source-indexed public overload（源索引公开入口，source 由 `indices_src` 指定，target 顺序扫描）。`Scalar=float`、连续 `weights_`、source index 全部有效、source 满足 `RVVXYZAoSFloatLayout`、target 满足 `RVVXYZNormalFloatLayout`、规模和 VLEN 验收条件成立时，当前默认进入 staged-gather / compressed-tail RVV 分流；失败时回到原 `ConstCloudIterator` 标量路径。`block-fused-abcd-ilp` production probe（生产探针）helper 仍保留给显式 probe / production detail A/B，不再作为默认优先 dispatch：Phase 031 的 public Std/RVV probe 为正向，但 Phase 032 的 production detail RVV-vs-RVV A/B 显示 block-fused 相对 staged 是 mixed / negative。
...
| production-source-indices staged-gather 5-run board summary | `test-rvv/registration/transformation_estimation_point_to_plane_lls_weighted/log/board/production_source_indices_staged_gather/summary.md` | source-indexed staged-gather / compressed-tail 的 repeated public Std/RVV 性能证据；Phase 033 后是当前默认生产证据。 |
| production-source-indices block-fused probe 5-run board summary | `test-rvv/registration/transformation_estimation_point_to_plane_lls_weighted/log/board/production_source_indices_block_fused_abcd_ilp_probe/summary.md`；`evidence_doctor.md` | Phase 031 source-indexed block-fused production probe 证据；6 个代表 case median 正向，但只证明 public RVV 快于 public scalar，不证明 block-fused 优于 staged。 |
| production-source-indices detail A/B | `test-rvv/registration/transformation_estimation_point_to_plane_lls_weighted/log/board/production_source_indices_detail_ba/analyze_rvv_ba.md` | Phase 032 根因证据；同一 production RVV binary 内 block-fused 对 staged mixed / negative，不能把 public Std/RVV 正向外推成 clean adoption。 |
...
| `estimateRigidTransformation(cloud_src, indices_src, cloud_tgt, matrix)`              | `source[indices_src[k]] + target[k]`              | 成员`weights_[k]`       | source-indexed staged-gather RVV；block-fused 只保留显式 probe。 |
...
### Source-indexed staged-gather default 与 block-fused explicit probe

Phase 033 后，source-indexed production 默认使用 staged-gather / compressed-tail helper。这个 helper 保留 valid-index scan 和 `uint32_t` staging，然后用 source gather、target stride load、weight load 生成 `a/b/c/d/nx/ny/nz`，再通过 `vcompress` 后 scalar tail accumulation 构造 normal-equation。`block-fused-abcd-ilp` helper 仍保留在 production detail 层，供 `production-source-indices-detail-ba` 和后续显式 probe 使用；它不再作为 public 默认优先路径。
...
estimateRigidTransformation(cloud_src, indices_src, cloud_tgt, matrix)
  ...
  -> detail::buildPointToPlaneLLSWeightedSourceIndicesStagedRVV(...)
...
单个 VL chunk 的 source-indexed staged-gather 默认流程是：
...
vsetvl_e32m2
  ...
  -> a/b/c/d formula
  -> vcompress kept lanes
  -> scalar tail accumulation 更新 normal-equation
```

block-fused probe helper 的单个 block 改用 source gather、target stride load 和 A/B/C/N block groups 维护向量 partial sums。Phase 032 的同边界 production RVV-vs-RVV A/B 显示它相对 staged 是 mixed / negative，因此当前只作为显式 probe / detail helper。source-indexed 标量路径每行通过 `ConstCloudIterator` 访问 source 和 target，再逐行累加 normal-equation。RVV 路径把 source 离散访问批量化，target 和 weight 保持连续 / 跨步加载。它没有改 solver，也没有改 weight finite 语义。非有限 point/normal 被 mask 掉；非有限 weight 仍参与计算。
...
这意味着当前 RVV 实现没有改变公开入口的对象状态，也没有改变 dual-indices 或 correspondences overload。full-cloud 路径用 block-reduction。source-indexed 默认路径用 valid-index scan + staged-gather / compressed-tail；block-fused 只作为显式 probe helper 保留。
...
| 行尾处理 | 每个有效点直接更新 normal-equation。 | `buildPointToPlaneLLSWeightedFullCloudBlockRVV` 按 A/B/C/N 四组维护 vector partial sums，组结束后 `vfredosum` 到 double normal-equation。 | 默认 staged-gather 用 `vcompress` 后 scalar tail 累加压缩行；block-fused probe helper 按 A/B/C/N 四组维护 vector partial sums。 | source-indexed 当前默认 staged-gather；block-fused 仅显式 probe。 | `run_test_candidates`、`run_test_source_indices`、production staged summary 和 detail A/B。 |
...
它先改变 full-cloud、`Scalar=float`、满足 layout/size/VLEN gate 的 weighted production implementation。Phase 031 又在 source-indexed public overload 上尝试同 family 的 block-fused production probe；Phase 032 的同边界 A/B 显示该 family 不适合做 source-indexed 默认优先路径，Phase 033 已把 source-indexed 默认退回 staged-gather。dual-indices 和 correspondences 仍不进入 production RVV。
...
| source-indexed production                    | adopted / staged default | staged-gather / compressed-tail 有 repeated public Std/RVV summary；Phase 032 detail A/B 显示 block-fused 对 staged mixed / negative，因此 block-fused 只保留显式 probe。 |
...
| source-indexed  | `source[indices_src[k]] + target[k]`。              | `weights_[k]` 连续 load。                       | `pcl::Indices` 输入计时前构造；production 内 valid-index scan、`uint32_t` staging、source gather、target stride load、`vcompress`、scalar tail、solve 和 matrix 计时；block-fused detail A/B 另含 A/B/C/N block groups。 | adopted / staged default。          |
...
source-indexed 当前默认 production performance 采用 staged-gather summary `production_source_indices_staged_gather/summary.md`。该组使用 65536 和 262144 点、5 runs、20 iterations 和 5 warm-up iterations：

| case | runs | 262144 median/min | 65536 median/min | 边界 |
| --- | ---: | ---: | ---: | --- |
| `pointnormal` | 5 | `2.33x / 1.99x` | `2.59x / 2.47x` | `PointNormal -> PointNormal` source gather。 |
| `pointxyz-to-pointnormal` | 5 | `2.22x / 2.18x` | `2.57x / 2.49x` | source generic xyz f32 AoS representative。 |
| `pointxyz-to-pointxyzinormal` | 5 | `2.37x / 2.28x` | `2.78x / 2.70x` | source + target generic layout representative。 |

这些 case 的 std/RVV 两侧都调用真实 source-indexed public overload。它们不覆盖 dual-indices、correspondences、invalid source index public API 行为、`Scalar=double` 或非连续权重。Phase 031 的 block-fused probe summary `production_source_indices_block_fused_abcd_ilp_probe/summary.md` 仍保留为历史 probe evidence：三类代表点型 262144 median/min 分别为 `1.64x / 1.17x`、`1.54x / 1.06x`、`1.69x / 1.41x`；Phase 032 detail A/B 说明这些 public Std/RVV 正向不能证明 block-fused 优于 staged。
...
| performance（性能）         | `production_dispatch_fused_abcd_ilp/summary.md`、`production_source_indices_staged_gather/summary.md`、`production_source_indices_block_fused_abcd_ilp_probe/summary.md` 和 `production_default_fused_abcd_ilp/trace_summary.md`。 | full-cloud 三类代表点型在 262144 点上 repeated std/RVV speedup 均正向；source-indexed staged-gather 三类代表点型在 65536 和 262144 点上 repeated std/RVV speedup 均正向；block-fused probe public Std/RVV 也正向但 detail A/B 对 staged mixed / negative；默认 RVV trace checksum 序列一致。 | 10-case diagnostic 不替代 production evidence；QEMU timing 不作性能结论；block-fused 不作为 source-indexed 默认 family。 |
...
| source-indexed invalid index gate miss                         | 不进入 staged-gather 或 block-fused probe RVV。 | 原 iterator 标量路径。                                         |
...
- source-indexed block-fused：保留为显式 production detail helper。重新考虑默认路径前，需要新的同边界 production detail A/B、source-indexed-specific asm attribution、binary identity 和 extended-run 同时转正。
