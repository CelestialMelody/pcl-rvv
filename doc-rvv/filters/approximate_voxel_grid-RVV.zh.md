...
- `PointT = pcl::PointXYZ`：保留专用快路径，RVV 产出 leaf/hash，随后用轻量
  `PointXYZHistoryEntry` 累加 `x/y/z/count`；
- 其它满足 `pcl::rvv::kRVVXYZPointCompatible<PointT>` 的点类型：使用 generic
  staged 路径，RVV 只产出 `LeafHash[]`，字段聚合、`downsample_all_data_` 和
  RGB/RGBA packing 继续复用原标量 `FieldList` / `flush` 语义。

```text
finite(x,y,z) -> floor(xyz * inverse_leaf_size) -> hash bucket
             -> scalar history collision/flush -> PointT centroid output
```

公开 API 不变。小规模、非 RVV 编译、非法 `histsize_` 和 XYZ layout gate 失败继续走原标量实现。

早期 `PointXYZ` 限定是 RVV 生产 helper 的输出语义边界，不是
`ApproximateVoxelGrid` 算法本身只能处理 `PointXYZ`。源码证据显示该类有真实泛型使用：

- `filters/src/approximate_voxel_grid.cpp` 用 `PCL_XYZ_POINT_TYPES` 预编译
  `ApproximateVoxelGrid`，覆盖 `PointXYZI`、`PointXYZRGBA`、`PointXYZRGB`、
  `PointXYZRGBNormal` 等常见 xyz 点类型；
- `test/keypoints/test_keypoints.cpp` 使用 `ApproximateVoxelGrid<PointXYZI>`；
- tracking tutorial 使用 `ApproximateVoxelGrid<PointXYZRGBA>`；
- `apps/src/openni_boundary_estimation.cpp` 使用
  `ApproximateVoxelGrid<PointXYZRGBNormal>`。

因此本轮采用的泛型化方向是：leaf/hash staging 泛化到 XYZ-compatible 点类型，泛型字段
聚合、`downsample_all_data_` 和 RGB/RGBA packing 继续复用标量状态机。不能把
`PointXYZHistoryEntry` 和 `flushPointXYZHistoryEntry` 直接套到 `PointXYZI` /
`PointXYZRGB`，否则会丢失或改变额外字段的 centroid 语义。
...
原实现随后用 `history_[hash]` 处理冲突、`flush`、`Eigen::VectorXf scratch`、RGB 解包、字段复制和 centroid 累加。本轮替换的是逐点 leaf/hash 计算：RVV 预计算同序 `LeafHash[]`，history bucket、冲突 flush、末尾 flush 和 centroid 累加保持标量状态机。`PointXYZ` 使用轻量专用 history；其它 XYZ-compatible `PointT` 使用原泛型 `history_`、`FieldList`、动态 scratch 和 RGB/RGBA 打包逻辑。

## 3. 覆盖范围与 fallback

| 项目 | 结论 |
| --- | --- |
| 覆盖点类型 | `PointXYZ` 专用快路径；其它满足 `pcl::rvv::kRVVXYZPointCompatible<PointT>` 的点类型走 generic staged 路径 |
| 覆盖数据形态 | 全云线性扫描，`histsize_` 为 2 的幂，`input_->size() >= 64` |
| 回退条件 | 小规模、非法 `histsize_`、非 RVV 编译、XYZ layout gate 失败 |
| history / flush | 仍为标量状态机，保证冲突输出顺序和 centroid 语义 |
| 泛型字段 / RGB | 生产已接入 generic staged 路径，但额外字段和 RGB/RGBA 聚合仍保持原 `Eigen::VectorXf scratch` + `FieldList` 标量语义 |
...
阶段 2：scalar history state machine
  按 LeafHash[] 顺序访问原 cloud[source_index]
  保持 bucket 冲突、flush 顺序和 centroid 累加语义
```

`LeafHash` 是阶段 1 和阶段 2 的边界数据：
...
生产实现保留一个 `PointT = pcl::PointXYZ` 专用快路径。`PointXYZ` 输出只需要
`x/y/z/count` centroid，使用 `PointXYZHistoryEntry` 可以把这条 RVV helper 的语义收窄到固定字段：
...
这样做避免把泛型字段聚合、RGB/RGBA 和动态 scratch 的维护边界误带入
`PointXYZ` 快路径。对 `PointXYZ` 而言，`downsample_all_data_` 没有额外字段可聚合，
因此这个窄状态机与标量输出语义一致。

generic staged 生产路径不使用 `PointXYZHistoryEntry`。它在 `applyFilterXYZStagedRVV`
中复用类成员 `history_`、原有 `flush`、`FieldList` 字段复制和 RGB/RGBA 特例；区别只是
逐点循环的 `finite + floor + hash` 来源从即时标量计算换成 `LeafHash[]`。因此
`PointXYZI` 的 intensity、`PointXYZRGB/RGBA` 的 packed 颜色字段和
`downsample_all_data_` 行为都由同一套标量聚合语义决定。
...
`approximate_voxel_grid_rvv` 是本文件内的实现命名空间，不改变公开 API。它集中放置
`ApproximateVoxelGrid` RVV staging 数据、数值 helper 和 `PointXYZ` 轻量 flush helper。
公开分流仍发生在类方法 `applyFilter` / `applyFilterPointXYZRVV` /
`applyFilterXYZStagedRVV` 中。

| 实体 | 类型 | 输入 / 输出 | 作用与边界 |
| --- | --- | --- | --- |
| `LeafHash` | staging 结构体 | `ix/iy/iz/hash/source_index` | 保存 RVV 阶段压缩后的有效点决策信息。`ix/iy/iz/hash` 来自原标量 leaf/hash 公式，`source_index` 指回原输入点，保证后续 centroid 累加读取同一份 cloud 数据。 |
...
| `computeXYZLeafHashes<PointT>` | RVV staging helper | `PointCloud<PointT> + inverse_leaf_size + histsize -> vector<LeafHash>` | 每个 VL chunk 读取 AoS `x/y/z`，计算 finite mask、floor、hash 和 source index，再用 `vcompress` 保序写出有效 lane。只覆盖 `pcl::rvv::kRVVXYZPointCompatible<PointT>`，且仅在 `n >= 64`、`histsize` 为 2 的幂、点数可用 `uint32_t` 表达时返回 true。 |
...
| `applyFilterPointXYZRVV` | 类内生产 helper | `PointCloud& output -> bool` | 调用 `computeXYZLeafHashes<PointXYZ>` 后，按 staging 顺序执行 `PointXYZ` 专用标量 bucket 冲突、flush 和 centroid 累加；覆盖条件不满足时返回 false，让公开入口回退 `applyFilterStd`。 |
| `applyFilterXYZStagedRVV` | 类内生产 helper | `PointCloud& output -> bool` | 调用 `computeXYZLeafHashes<PointT>` 后，按 staging 顺序执行原泛型 `history_`、`FieldList`、RGB/RGBA 和 `flush` 逻辑；覆盖条件不满足时返回 false，让公开入口回退 `applyFilterStd`。 |

`computeXYZLeafHashes` 的输出数组是这个实现的核心边界。它没有复制 `x/y/z`，只保存后续状态机做同序判断所需的最小信息：
...
需要注意的是，`PointXYZHistoryEntry` 是局部 history 表，不复用类成员 `history_`。这不改变公开输出：每次 `applyFilterPointXYZRVV` 都重新建立局部 history，末尾完整 flush，输出 `width/height/is_dense` 与标量路径一致。generic staged 路径和标量 fallback 仍使用原 `history_` 和 `he` 结构，泛型状态不会被 `PointXYZ` 窄路径污染。
...
- std/RVV 两套构建均通过 10 个测试，覆盖负数 floor、finite mask、RVV/Std leaf-hash 对拍、full `PointXYZ` 诊断对拍、生产 `PointXYZ` 输出对拍，以及生产 `PointXYZI` / `PointXYZRGB` / `PointXYZRGBA` 与强制标量输出对拍。
...
| `approx voxel full diag 64K` | 64K `PointXYZ`，无 invalid，`histsize=512` | RVV leaf/hash + 标量 bucket/flush/centroid | 1.63x | bucket、flush 和 centroid 累加纳入后仍有整体收益 |
| `approx voxel full diag 1M` | 1M `PointXYZ`，无 invalid，`histsize=512` | RVV leaf/hash + 标量 bucket/flush/centroid | 1.64x | 大规模 full `PointXYZ` 形态下收益没有被下游标量工作完全抵消 |
| `approx voxel finite full diag 1M` | 1M `PointXYZ`，含 NaN / Inf，`histsize=512` | RVV finite leaf/hash + 标量 bucket/flush/centroid | 1.62x | invalid 跳过与 full diagnostic 同时存在时仍保持收益 |
| `approx voxel production pointxyz 64K` | 64K `PointXYZ`，调用生产 `ApproximateVoxelGrid<PointXYZ>::filter(output)` | 生产 `PointXYZ` 快路径 | 1.91x | 证明接入生产入口后收益成立 |
| `approx voxel production pointxyzi 64K` | 64K `PointXYZI`，调用生产 `ApproximateVoxelGrid<PointXYZI>::filter(output)` | generic staged 生产路径 | 1.21x | RVV leaf/hash + 标量 intensity 聚合有板卡收益 |
| `approx voxel production pointxyzrgb 64K` | 64K `PointXYZRGB`，调用生产 `ApproximateVoxelGrid<PointXYZRGB>::filter(output)` | generic staged 生产路径 | 1.14x | RVV leaf/hash + 标量 RGB packing 有板卡收益 |
| `approx voxel production pointxyzrgba 64K` | 64K `PointXYZRGBA`，调用生产 `ApproximateVoxelGrid<PointXYZRGBA>::filter(output)` | generic staged 生产路径 | 1.12x | RVV leaf/hash + 标量 RGBA/RGB packing 有板卡收益 |

## 8. 结论

`ApproximateVoxelGrid<PointXYZ>::filter(output)` 已接入 RVV 生产快路径；其它满足
`pcl::rvv::kRVVXYZPointCompatible<PointT>` 的点类型已接入 generic staged 生产路径。
Milkv-Jupiter 上生产 `PointXYZ` case 为 `1.91x`，`PointXYZI` 为 `1.21x`，
`PointXYZRGB` 为 `1.14x`，`PointXYZRGBA` 为 `1.12x`。这说明在真实板卡上，
leaf/hash staging 的收益可以覆盖泛型字段标量聚合的额外成本。

生产覆盖边界仍然明确：点数 `>=64`、`histsize_` 为 2 的幂、RVV 编译且
`PointT` 满足 `pcl::rvv::kRVVXYZPointCompatible<PointT>`。不满足条件时回退
`applyFilterStd`。本轮没有把 intensity、normal、curvature 或 RGB packing 本身 RVV
化；这些字段仍由原标量 `FieldList` / RGB/RGBA 逻辑维护。后续如果考虑额外字段 RVV
累加，应作为独立 topic，用专项语义对拍和板卡 bench 重新证明。
